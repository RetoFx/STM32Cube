...
/** @defgroup SAI_Private_Functions  SAI Private Functions
  * @{
  */
static void SAI_FillFifo(SAI_HandleTypeDef * const hsai);
static uint32_t SAI_InterruptFlag(SAI_HandleTypeDef const * const hsai, SAI_ModeTypedef mode);
static HAL_StatusTypeDef SAI_InitI2S(SAI_HandleTypeDef * const hsai, uint32_t protocol, uint32_t datasize, uint32_t nbslot);
static HAL_StatusTypeDef SAI_InitPCM(SAI_HandleTypeDef * const hsai, uint32_t protocol, uint32_t datasize, uint32_t nbslot);

static HAL_StatusTypeDef SAI_Disable(SAI_HandleTypeDef * const hsai);
static void SAI_Transmit_IT8Bit(SAI_HandleTypeDef * const hsai);
static void SAI_Transmit_IT16Bit(SAI_HandleTypeDef * const hsai);
static void SAI_Transmit_IT32Bit(SAI_HandleTypeDef * const hsai);
static void SAI_Receive_IT8Bit(SAI_HandleTypeDef * const hsai);
static void SAI_Receive_IT16Bit(SAI_HandleTypeDef * const hsai);
static void SAI_Receive_IT32Bit(SAI_HandleTypeDef * const hsai);

static void SAI_DMATxCplt(DMA_HandleTypeDef * const hdma);
static void SAI_DMATxHalfCplt(DMA_HandleTypeDef * const hdma);
static void SAI_DMARxCplt(DMA_HandleTypeDef * const hdma);
static void SAI_DMARxHalfCplt(DMA_HandleTypeDef * const hdma);
static void SAI_DMAError(DMA_HandleTypeDef * const hdma);
static void SAI_DMAAbort(DMA_HandleTypeDef * const hdma);
/**
  * @}
  */
...
HAL_StatusTypeDef HAL_SAI_Init(SAI_HandleTypeDef * const hsai)
{
  ...
  if (hsai->Init.AudioFrequency != SAI_AUDIO_FREQUENCY_MCKDIV)
  {
    ...
    if (hsai->Init.Protocol == SAI_SPDIF_PROTOCOL)
    {
      hsai->Init.Mckdiv = hsai->Init.Mckdiv >> 1U;
    }
  }
...
HAL_StatusTypeDef HAL_SAI_DeInit(SAI_HandleTypeDef * const hsai)
{
  ...
  hsai->Instance->IMR = 0;
  hsai->Instance->CLRFR = 0xFFFFFFFFUL;
...
/**
  ...
  * @retval None
  */
__weak void HAL_SAI_MspInit(SAI_HandleTypeDef * const hsai)
{
...
/**
  ...
  * @retval None
  */
__weak void HAL_SAI_MspDeInit(SAI_HandleTypeDef * const hsai)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hsai);
    ...
    while (hsai->XferCount > 0U)
    {
      ...
      if ((hsai->Instance->SR & SAI_xSR_FLVL) != SAI_FIFOSTATUS_FULL)
      {
        ...
        else if (hsai->Init.DataSize <= SAI_DATASIZE_16)
        {
          ...
          hsai->pBuffPtr++;
          temp |= ((uint32_t)(*hsai->pBuffPtr) << 8U);
          hsai->pBuffPtr++;
        ...
        else
        {
          ...
          temp |= ((uint32_t)(*hsai->pBuffPtr) << 8U);
          hsai->pBuffPtr++;
          temp |= ((uint32_t)(*hsai->pBuffPtr) << 16U);
          hsai->pBuffPtr++;
          temp |= ((uint32_t)(*hsai->pBuffPtr) << 24U);
          hsai->pBuffPtr++;
          hsai->Instance->DR = temp;
        }
        hsai->XferCount--;
    ...
    while (hsai->XferCount > 0U)
    {
      if ((hsai->Instance->SR & SAI_xSR_FLVL) != SAI_FIFOSTATUS_EMPTY)
      {
        ...
        else
        {
          ...
          *hsai->pBuffPtr = (uint8_t)(temp >> 8U);
          hsai->pBuffPtr++;
          *hsai->pBuffPtr = (uint8_t)(temp >> 16U);
          hsai->pBuffPtr++;
          *hsai->pBuffPtr = (uint8_t)(temp >> 24U);
          hsai->pBuffPtr++;
        }
        hsai->XferCount--;
      }
...
/**
  ...
  * @retval None
  */
HAL_SAI_WEAK_CALLBACK(void, HAL_SAI_TxCpltCallback)(SAI_HandleTypeDef * const hsai)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hsai);
...
/**
  ...
  * @retval None
  */
HAL_SAI_WEAK_CALLBACK(void, HAL_SAI_TxHalfCpltCallback)(SAI_HandleTypeDef * const hsai)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hsai);
...
/**
  ...
  * @retval None
  */
HAL_SAI_WEAK_CALLBACK(void, HAL_SAI_RxCpltCallback)(SAI_HandleTypeDef * const hsai)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hsai);
...
/**
  ...
  * @retval None
  */
HAL_SAI_WEAK_CALLBACK(void, HAL_SAI_RxHalfCpltCallback)(SAI_HandleTypeDef * const hsai)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hsai);
...
/**
  ...
  * @retval None
  */
HAL_SAI_WEAK_CALLBACK(void, HAL_SAI_ErrorCallback)(SAI_HandleTypeDef * const hsai)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hsai);
...
static void SAI_DMATxCplt(DMA_HandleTypeDef * const hdma)
{
  SAI_HandleTypeDef * const hsai = (SAI_HandleTypeDef *)((DMA_HandleTypeDef *)hdma)->Parent;
