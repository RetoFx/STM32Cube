...
/** @addtogroup DCMI_Private_Functions DCMI Private Functions
  * @{
  */
static void       DCMI_DMAXferCplt(DMA_HandleTypeDef * const hdma);
static void       DCMI_DMAHalfXferCplt(DMA_HandleTypeDef * const hdma);
static void       DCMI_DMAError(DMA_HandleTypeDef * const hdma);
...
/**
  ...
  * @retval None
  */
__weak void HAL_DCMI_MspInit(DCMI_HandleTypeDef * const hdcmi)
{
...
/**
  ...
  * @retval None
  */
__weak void HAL_DCMI_MspDeInit(DCMI_HandleTypeDef * const hdcmi)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hdcmi);
...
/**
  ...
  * @note  In case of length larger than 65535 (0xFFFFU is the DMA maximum transfer length),
  ...
  * @note  For length less than 0xFFFFU (DMA maximum transfer length) and in snapshot mode,
...
HAL_StatusTypeDef HAL_DCMI_Start_DMA(DCMI_HandleTypeDef * const hdcmi, uint32_t DCMI_Mode, uint32_t pData, uint32_t Length)
{
  ...
  if(Length <= 0xFFFFU)
  {
    hdcmi->XferCount = 0U; /* Mark as direct transfer from DCMI_DR register to final destination buffer */
...
/**
  ...
  * @retval None
  */
HAL_DMCI_WEAK_CALLBACK(void, HAL_DCMI_ErrorCallback)(DCMI_HandleTypeDef * const hdcmi)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hdcmi);
...
/**
  ...
  * @retval None
  */
HAL_DMCI_WEAK_CALLBACK(void, HAL_DCMI_LineEventCallback)(DCMI_HandleTypeDef * const hdcmi)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hdcmi);
...
/**
  ...
  * @retval None
  */
HAL_DMCI_WEAK_CALLBACK(void, HAL_DCMI_VsyncEventCallback)(DCMI_HandleTypeDef * const hdcmi)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hdcmi);
...
/**
  ...
  * @retval None
  */
HAL_DMCI_WEAK_CALLBACK(void, HAL_DCMI_FrameEventCallback)(DCMI_HandleTypeDef * const hdcmi)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hdcmi);
...
/**
  ...
  * @note For all the parameters, the actual value is the input data + 1 (e.g. YSize = 0x0U means 1 line,
  *       YSize = 0x1U means 2 lines, ...)
  * @retval HAL status
  */
...
/**
  ...
  *       larger than 0xFFFFU (DMA maximum transfer length), this API initiates
...
static void DCMI_DMAXferCplt(DMA_HandleTypeDef * const hdma)
{
  ...
  DCMI_HandleTypeDef * const hdcmi = ( DCMI_HandleTypeDef * )((DMA_HandleTypeDef * )hdma)->Parent;


  if(hdcmi->XferCount != 0U)
  {
    if (hdcmi->XferCount == 0xBEBEU)
    {
...
/**
  ...
  * @note In the case of a frame size larger than the maximum DMA transfer length (0xFFFFU),
  *       the transfer from DCMI DR register to the final output buffer is carried out by a sequence
  *       of intermediate sub-copies to temporary buffers of size less than 0xFFFFU.
  *       To optimize the number of DMA transfers, the API computes the temporary buffer
  *       size so that the latter is an even number less than 0xFFFFU, that divides the final
...
static uint32_t DCMI_TransferSize(uint32_t InputSize)
{
  ...
  static uint32_t const PrimeArray[NPRIME] = { 1UL,  2UL,  3UL,  5UL,
                                7UL, 11UL, 13UL, 17UL,
                               19UL, 23UL, 29UL, 31UL,
                               37UL, 41UL, 43UL, 47UL};
  ...
  /*  Search for the biggest even divisor less or equal to 0xFFFEU = 65534 */
  aPrime[1U] -= 1U; /* output is initialized to 2, so don't count divider 2 twice */
