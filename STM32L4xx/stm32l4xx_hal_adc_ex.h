...
typedef struct
{
  ...
  uint32_t InjectedOffset;                /*!< Defines the offset to be subtracted from the raw converted data.
                                               ...
                                               parameter must be a number between Min_Data = 0x000U and Max_Data = 0xFFFU,
                                               0x3FFU, 0xFFU or 0x3FU respectively.
...
/**
  ...
  * @param __HANDLE__ ADC handle.
  * @note When DMSDFM configuration is not supported, the macro systematically reports 0x0U (i.e disabled).
...
void                    HAL_ADCEx_InjectedConvCpltCallback(ADC_HandleTypeDef * const hadc);
void                    HAL_ADCEx_InjectedQueueOverflowCallback(ADC_HandleTypeDef * const hadc);
void                    HAL_ADCEx_LevelOutOfWindow2Callback(ADC_HandleTypeDef * const hadc);
void                    HAL_ADCEx_LevelOutOfWindow3Callback(ADC_HandleTypeDef * const hadc);
void                    HAL_ADCEx_EndOfSamplingCallback(ADC_HandleTypeDef * const hadc);
...
#if defined(ADC_MULTIMODE_SUPPORT)
HAL_StatusTypeDef       HAL_ADCEx_MultiModeConfigChannel(ADC_HandleTypeDef * const hadc,
                                                         ADC_MultiModeTypeDef const * const pMultimode);
#endif /* ADC_MULTIMODE_SUPPORT */
