...
#define SD_INIT_FREQ                   400000UL   /* Initialization phase : 400 kHz max */
#define SD_NORMAL_SPEED_FREQ           25000000UL /* Normal speed phase : 25 MHz max */
#define SD_HIGH_SPEED_FREQ             50000000UL /* High speed phase : 50 MHz max */
/**
  * @}
  */
...
/** @defgroup SD_Private_Functions SD Private Functions
  * @{
  */
static uint32_t SD_InitCard       (SD_HandleTypeDef * const hsd);
static uint32_t SD_PowerON        (SD_HandleTypeDef * const hsd);
static uint32_t SD_SendSDStatus   (SD_HandleTypeDef * const hsd, uint32_t * const pSDstatus);
static uint32_t SD_SendStatus     (SD_HandleTypeDef const * const hsd, uint32_t * const pCardStatus);
static uint32_t SD_WideBus_Enable (SD_HandleTypeDef * const hsd);
static uint32_t SD_WideBus_Disable(SD_HandleTypeDef * const hsd);
static uint32_t SD_FindSCR        (SD_HandleTypeDef * const hsd, uint32_t * const pSCR);
static void     SD_PowerOFF       (SD_HandleTypeDef * const hsd);
static void     SD_Write_IT       (SD_HandleTypeDef * const hsd);
static void     SD_Read_IT        (SD_HandleTypeDef * const hsd);
#if !defined(STM32L4P5xx) && !defined(STM32L4Q5xx) && !defined(STM32L4R5xx) && !defined(STM32L4R7xx) && !defined(STM32L4R9xx) && !defined(STM32L4S5xx) && !defined(STM32L4S7xx) && !defined(STM32L4S9xx)
static void     SD_DMATransmitCplt(DMA_HandleTypeDef * const hdma);
static void     SD_DMAReceiveCplt (DMA_HandleTypeDef * const hdma);
static void     SD_DMAError       (DMA_HandleTypeDef * const hdma);
static void     SD_DMATxAbort     (DMA_HandleTypeDef * const hdma);
static void     SD_DMARxAbort     (DMA_HandleTypeDef * const hdma);
#else
static uint32_t SD_UltraHighSpeed (SD_HandleTypeDef * const hsd);
static uint32_t SD_DDR_Mode       (SD_HandleTypeDef * const hsd);
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_SD_Init(SD_HandleTypeDef * const hsd)
{
#if defined(STM32L4P5xx) || defined(STM32L4Q5xx) || defined(STM32L4R5xx) || defined(STM32L4R7xx) || defined(STM32L4R9xx) || defined(STM32L4S5xx) || defined(STM32L4S7xx) || defined(STM32L4S9xx)
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_SD_InitCard(SD_HandleTypeDef * const hsd)
{
  uint32_t errorstate;
  SD_InitTypeDef Init;
  uint32_t sdmmc_clk;
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_SD_DeInit(SD_HandleTypeDef * const hsd)
{
  /* Check the SD handle allocation */
  if(hsd == NULL)
  {
...
/**
  ...
  * @retval None
  */
__weak void HAL_SD_MspInit(SD_HandleTypeDef * const hsd)
{
...
/**
  ...
  * @retval None
  */
__weak void HAL_SD_MspDeInit(SD_HandleTypeDef * const hsd)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hsd);
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_SD_ReadBlocks(SD_HandleTypeDef * const hsd, uint8_t * const pData, uint32_t BlockAdd, uint32_t NumberOfBlocks, uint32_t Timeout)
{
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_SD_WriteBlocks(SD_HandleTypeDef * const hsd, uint8_t const * const pData, uint32_t BlockAdd, uint32_t NumberOfBlocks, uint32_t Timeout)
{
  SDMMC_DataInitTypeDef config;
  uint32_t errorstate;
  uint32_t tickstart = HAL_GetTick();
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_SD_ReadBlocks_IT(SD_HandleTypeDef * const hsd, uint8_t * const pData, uint32_t BlockAdd, uint32_t NumberOfBlocks)
{
  SDMMC_DataInitTypeDef config;
  uint32_t errorstate;
  uint32_t add = BlockAdd;
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_SD_WriteBlocks_IT(SD_HandleTypeDef * const hsd, uint8_t const * const pData, uint32_t BlockAdd, uint32_t NumberOfBlocks)
{
  SDMMC_DataInitTypeDef config;
  uint32_t errorstate;
  uint32_t add = BlockAdd;
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_SD_ReadBlocks_DMA(SD_HandleTypeDef * const hsd, uint8_t * const pData, uint32_t BlockAdd, uint32_t NumberOfBlocks)
{
  SDMMC_DataInitTypeDef config;
  uint32_t errorstate;
  uint32_t add = BlockAdd;
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_SD_WriteBlocks_DMA(SD_HandleTypeDef * const hsd, uint8_t const * const pData, uint32_t BlockAdd, uint32_t NumberOfBlocks)
{
  SDMMC_DataInitTypeDef config;
  uint32_t errorstate;
  uint32_t add = BlockAdd;
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_SD_Erase(SD_HandleTypeDef * const hsd, uint32_t BlockStartAdd, uint32_t BlockEndAdd)
{
  uint32_t errorstate;
...
/**
  ...
  * @retval None
  */
void HAL_SD_IRQHandler(SD_HandleTypeDef * const hsd)
{
  uint32_t errorstate;
...
/**
  ...
  * @retval HAL state
  */
HAL_SD_StateTypeDef HAL_SD_GetState(SD_HandleTypeDef const * const hsd)
{
  return hsd->State;
}
...
*/
uint32_t HAL_SD_GetError(SD_HandleTypeDef const * const hsd)
{
...
/**
  ...
  * @retval None
  */
HAL_SD_WEAK_CALLBACK(void, HAL_SD_TxCpltCallback)(SD_HandleTypeDef * const hsd)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hsd);
...
/**
  ...
  * @retval None
  */
HAL_SD_WEAK_CALLBACK(void, HAL_SD_RxCpltCallback)(SD_HandleTypeDef * const hsd)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hsd);
...
/**
  ...
  * @retval None
  */
HAL_SD_WEAK_CALLBACK(void, HAL_SD_ErrorCallback)(SD_HandleTypeDef * const hsd)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hsd);
...
/**
  ...
  * @retval None
  */
HAL_SD_WEAK_CALLBACK(void, HAL_SD_AbortCallback)(SD_HandleTypeDef * const hsd)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hsd);
...
/**
  ...
  * @retval status
  */
HAL_StatusTypeDef HAL_SD_RegisterCallback(SD_HandleTypeDef * const hsd, HAL_SD_CallbackIDTypeDef CallbackID, pSD_CallbackTypeDef pCallback)
{
...
/**
  ...
  * @retval status
  */
HAL_StatusTypeDef HAL_SD_UnRegisterCallback(SD_HandleTypeDef * const hsd, HAL_SD_CallbackIDTypeDef CallbackID)
{
  HAL_StatusTypeDef status = HAL_OK;
...
/**
  ...
  * @retval status
  */
HAL_StatusTypeDef HAL_SD_RegisterTransceiverCallback(SD_HandleTypeDef * const hsd, pSD_TransceiverCallbackTypeDef pCallback)
{
  HAL_StatusTypeDef status = HAL_OK;
...
/**
  ...
  * @retval status
  */
HAL_StatusTypeDef HAL_SD_UnRegisterTransceiverCallback(SD_HandleTypeDef * const hsd)
{
  HAL_StatusTypeDef status = HAL_OK;
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_SD_GetCardCID(SD_HandleTypeDef * const hsd, HAL_SD_CardCIDTypeDef * const pCID)
{
  pCID->ManufacturerID = (uint8_t)((hsd->CID[0U] & 0xFF000000UL) >> 24U);

  pCID->OEM_AppliID = (uint16_t)((hsd->CID[0U] & 0x00FFFF00UL) >> 8U);

  pCID->ProdName1 = (((hsd->CID[0U] & 0x000000FFUL) << 24U) | ((hsd->CID[1U] & 0xFFFFFF00UL) >> 8U));

  pCID->ProdName2 = (uint8_t)(hsd->CID[1U] & 0x000000FFUL);

  pCID->ProdRev = (uint8_t)((hsd->CID[2U] & 0xFF000000UL) >> 24U);

  pCID->ProdSN = (((hsd->CID[2U] & 0x00FFFFFFUL) << 8U) | ((hsd->CID[3U] & 0xFF000000UL) >> 24U));

  pCID->Reserved1 = (uint8_t)((hsd->CID[3U] & 0x00F00000UL) >> 20U);

  pCID->ManufactDate = (uint16_t)((hsd->CID[3U] & 0x000FFF00UL) >> 8U);

  pCID->CID_CRC = (uint8_t)((hsd->CID[3U] & 0x000000FEUL) >> 1U);
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_SD_GetCardCSD(SD_HandleTypeDef * const hsd, HAL_SD_CardCSDTypeDef * const pCSD)
{
  pCSD->CSDStruct = (uint8_t)((hsd->CSD[0U] & 0xC0000000UL) >> 30U);

  pCSD->SysSpecVersion = (uint8_t)((hsd->CSD[0U] & 0x3C000000UL) >> 26U);

  pCSD->Reserved1 = (uint8_t)((hsd->CSD[0U] & 0x03000000UL) >> 24U);

  pCSD->TAAC = (uint8_t)((hsd->CSD[0U] & 0x00FF0000UL) >> 16U);

  pCSD->NSAC = (uint8_t)((hsd->CSD[0U] & 0x0000FF00UL) >> 8U);

  pCSD->MaxBusClkFrec = (uint8_t)(hsd->CSD[0U] & 0x000000FFUL);

  pCSD->CardComdClasses = (uint16_t)((hsd->CSD[1U] & 0xFFF00000UL) >> 20U);

  pCSD->RdBlockLen = (uint8_t)((hsd->CSD[1U] & 0x000F0000UL) >> 16U);

  pCSD->PartBlockRead   = (uint8_t)((hsd->CSD[1U] & 0x00008000UL) >> 15U);

  pCSD->WrBlockMisalign = (uint8_t)((hsd->CSD[1U] & 0x00004000UL) >> 14U);

  pCSD->RdBlockMisalign = (uint8_t)((hsd->CSD[1U] & 0x00002000UL) >> 13U);

  pCSD->DSRImpl = (uint8_t)((hsd->CSD[1U] & 0x00001000UL) >> 12U);

  pCSD->Reserved2 = 0U; /*!< Reserved */

  if(hsd->SdCard.CardType == CARD_SDSC)
  {
    pCSD->DeviceSize = (((hsd->CSD[1U] & 0x000003FFUL) << 2U) | ((hsd->CSD[2U] & 0xC0000000UL) >> 30U));

    pCSD->MaxRdCurrentVDDMin = (uint8_t)((hsd->CSD[2U] & 0x38000000UL) >> 27U);

    pCSD->MaxRdCurrentVDDMax = (uint8_t)((hsd->CSD[2U] & 0x07000000UL) >> 24U);

    pCSD->MaxWrCurrentVDDMin = (uint8_t)((hsd->CSD[2U] & 0x00E00000UL) >> 21U);

    pCSD->MaxWrCurrentVDDMax = (uint8_t)((hsd->CSD[2U] & 0x001C0000UL) >> 18U);

    pCSD->DeviceSizeMul = (uint8_t)((hsd->CSD[2U] & 0x00038000UL) >> 15U);
  ...
  {
    /* Byte 7 */
    pCSD->DeviceSize = (((hsd->CSD[1U] & 0x0000003FUL) << 16U) | ((hsd->CSD[2U] & 0xFFFF0000UL) >> 16U));
  ...
  pCSD->EraseGrSize = (uint8_t)((hsd->CSD[2U] & 0x00004000UL) >> 14U);

  pCSD->EraseGrMul = (uint8_t)((hsd->CSD[2U] & 0x00003F80UL) >> 7U);

  pCSD->WrProtectGrSize = (uint8_t)(hsd->CSD[2U] & 0x0000007FUL);

  pCSD->WrProtectGrEnable = (uint8_t)((hsd->CSD[3U] & 0x80000000UL) >> 31U);

  pCSD->ManDeflECC = (uint8_t)((hsd->CSD[3U] & 0x60000000UL) >> 29U);

  pCSD->WrSpeedFact = (uint8_t)((hsd->CSD[3U] & 0x1C000000UL) >> 26U);

  pCSD->MaxWrBlockLen= (uint8_t)((hsd->CSD[3U] & 0x03C00000UL) >> 22U);

  pCSD->WriteBlockPaPartial = (uint8_t)((hsd->CSD[3U] & 0x00200000UL) >> 21U);

  pCSD->Reserved3 = 0;

  pCSD->ContentProtectAppli = (uint8_t)((hsd->CSD[3U] & 0x00010000UL) >> 16U);

  pCSD->FileFormatGroup = (uint8_t)((hsd->CSD[3U] & 0x00008000UL) >> 15U);

  pCSD->CopyFlag = (uint8_t)((hsd->CSD[3U] & 0x00004000UL) >> 14U);

  pCSD->PermWrProtect = (uint8_t)((hsd->CSD[3U] & 0x00002000UL) >> 13U);

  pCSD->TempWrProtect = (uint8_t)((hsd->CSD[3U] & 0x00001000UL) >> 12U);

  pCSD->FileFormat = (uint8_t)((hsd->CSD[3U] & 0x00000C00UL) >> 10U);

  pCSD->ECC= (uint8_t)((hsd->CSD[3U] & 0x00000300UL) >> 8U);

  pCSD->CSD_CRC = (uint8_t)((hsd->CSD[3U] & 0x000000FEUL) >> 1U);
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_SD_GetCardStatus(SD_HandleTypeDef * const hsd, HAL_SD_CardStatusTypeDef * const pStatus)
{
  ...
  {
    pStatus->DataBusWidth = (uint8_t)((sd_status[0U] & 0xC0U) >> 6U);

    pStatus->SecuredMode = (uint8_t)((sd_status[0U] & 0x20U) >> 5U);

    pStatus->CardType = (uint16_t)(((sd_status[0U] & 0x00FF0000UL) >> 8U) | ((sd_status[0U] & 0xFF000000UL) >> 24U));

    pStatus->ProtectedAreaSize = (((sd_status[1U] & 0xFFU) << 24U)    | ((sd_status[1U] & 0xFF00U) << 8U) |
                                  ((sd_status[1U] & 0xFF0000UL) >> 8U) | ((sd_status[1U] & 0xFF000000UL) >> 24U));

    pStatus->SpeedClass = (uint8_t)(sd_status[2U] & 0xFFU);

    pStatus->PerformanceMove = (uint8_t)((sd_status[2U] & 0xFF00U) >> 8U);

    pStatus->AllocationUnitSize = (uint8_t)((sd_status[2U] & 0xF00000UL) >> 20U);

    pStatus->EraseSize = (uint16_t)(((sd_status[2U] & 0xFF000000UL) >> 16U) | (sd_status[3U] & 0xFFU));

    pStatus->EraseTimeout = (uint8_t)((sd_status[3U] & 0xFC00U) >> 10U);

    pStatus->EraseOffset = (uint8_t)((sd_status[3U] & 0x0300U) >> 8U);

#if defined(STM32L4P5xx) || defined(STM32L4Q5xx) || defined(STM32L4R5xx) || defined(STM32L4R7xx) || defined(STM32L4R9xx) || defined(STM32L4S5xx) || defined(STM32L4S7xx) || defined(STM32L4S9xx)
    pStatus->UhsSpeedGrade = (uint8_t)((sd_status[3U] & 0x00F0U) >> 4U);
    pStatus->UhsAllocationUnitSize = (uint8_t)(sd_status[3U] & 0x000FU) ;
    pStatus->VideoSpeedClass = (uint8_t)((sd_status[4U] & 0xFF000000UL) >> 24U);
#endif
  }
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_SD_GetCardInfo(SD_HandleTypeDef * const hsd, HAL_SD_CardInfoTypeDef * const pCardInfo)
{
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_SD_ConfigWideBusOperation(SD_HandleTypeDef * const hsd, uint32_t WideMode)
{
...
HAL_StatusTypeDef HAL_SD_ConfigSpeedBusOperation(SD_HandleTypeDef * const hsd, uint32_t SpeedMode)
{
  uint32_t tickstart;
  uint32_t errorstate;
  HAL_StatusTypeDef status = HAL_OK;
  ...
  if(hsd->Init.Transceiver == SDMMC_TRANSCEIVER_ENABLE)
  {
    switch (SpeedMode)
    {
      case SDMMC_SPEED_MODE_AUTO:
      {
        ...
        {
          hsd->Instance->CLKCR |= 0x00100000UL;
...
/**
  ...
  * @retval Card state
  */
HAL_SD_CardStateTypeDef HAL_SD_GetCardState(SD_HandleTypeDef * const hsd)
{
  uint32_t cardstate;
  uint32_t errorstate;
  uint32_t resp1 = 0;
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_SD_Abort(SD_HandleTypeDef * const hsd)
{
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_SD_Abort_IT(SD_HandleTypeDef * const hsd)
{
  HAL_SD_CardStateTypeDef CardState;
#if !defined(STM32L4P5xx) && !defined(STM32L4Q5xx) && !defined(STM32L4R5xx) && !defined(STM32L4R7xx) && !defined(STM32L4R9xx) && !defined(STM32L4S5xx) && !defined(STM32L4S7xx) && !defined(STM32L4S9xx)
  uint32_t context = hsd->Context;
...
static void SD_DMATransmitCplt(DMA_HandleTypeDef * const hdma)
{
  SD_HandleTypeDef * const hsd = (SD_HandleTypeDef * )(hdma->Parent);
...
static void SD_DMAReceiveCplt(DMA_HandleTypeDef * const hdma)
{
  ...
  if(hsd->Context == (SD_CONTEXT_READ_MULTIPLE_BLOCK | SD_CONTEXT_DMA))
  {
    ...
    {
      hsd->ErrorCode |= errorstate;
#if (USE_HAL_SD_REGISTER_CALLBACKS == 1U)
      hsd->ErrorCallback(hsd);
#else
      HAL_SD_ErrorCallback(hsd);
#endif
...
static uint32_t SD_PowerON(SD_HandleTypeDef * const hsd)
{
  ...
  /* SD CARD */
  /* Send ACMD41 SD_APP_OP_COND with Argument 0x80100000UL */
...
static uint32_t SD_FindSCR(SD_HandleTypeDef * const hsd, uint32_t * const pSCR)
{
  ...
  while(!__HAL_SD_GET_FLAG(hsd, SDMMC_FLAG_RXOVERR | SDMMC_FLAG_DCRCFAIL | SDMMC_FLAG_DTIMEOUT | SDMMC_FLAG_DBCKEND | SDMMC_FLAG_DATAEND))
  {
    ...
    {
      tempscr[0U] = SDMMC_ReadFIFO(hsd->Instance);
      tempscr[1U] = SDMMC_ReadFIFO(hsd->Instance);
      index++;
    }
  ...
  else
  {
    ...
    *scr = (((tempscr[1U] & SDMMC_0TO7BITS) << 24U)  | ((tempscr[1U] & SDMMC_8TO15BITS) << 8U) |
            ((tempscr[1U] & SDMMC_16TO23BITS) >> 8U) | ((tempscr[1U] & SDMMC_24TO31BITS) >> 24U));
    scr++;
    *scr = (((tempscr[0U] & SDMMC_0TO7BITS) << 24U)  | ((tempscr[0U] & SDMMC_8TO15BITS) << 8U) |
...
/**
  ...
  * @retval SD Card error state
  */
uint32_t SD_HighSpeed(SD_HandleTypeDef * const hsd)
{
  uint32_t errorstate = HAL_SD_ERROR_NONE;
  SDMMC_DataInitTypeDef sdmmc_datainitstructure;
  uint32_t SD_hs[16U]  = {0};
