...
#define MMC_INIT_FREQ                   400000UL   /* Initialization phase : 400 kHz max */
#define MMC_HIGH_SPEED_FREQ             52000000UL /* High speed phase : 52 MHz max */
/**
  * @}
  */
...
/** @defgroup MMC_Private_Functions MMC Private Functions
  * @{
  */
static uint32_t MMC_InitCard(MMC_HandleTypeDef * const hmmc);
static uint32_t MMC_PowerON(MMC_HandleTypeDef * const hmmc);
static uint32_t MMC_SendStatus(MMC_HandleTypeDef * const hmmc, uint32_t * const pCardStatus);
static void     MMC_PowerOFF(MMC_HandleTypeDef * const hmmc);
static void     MMC_Write_IT(MMC_HandleTypeDef * const hmmc);
static void     MMC_Read_IT(MMC_HandleTypeDef * const hmmc);
#if !defined(STM32L4P5xx) && !defined(STM32L4Q5xx) && !defined(STM32L4R5xx) && !defined(STM32L4R7xx) && !defined(STM32L4R9xx) && !defined(STM32L4S5xx) && !defined(STM32L4S7xx) && !defined(STM32L4S9xx)
static void     MMC_DMATransmitCplt(DMA_HandleTypeDef * const hdma);
static void     MMC_DMAReceiveCplt(DMA_HandleTypeDef * const hdma);
static void     MMC_DMAError(DMA_HandleTypeDef * const hdma);
static void     MMC_DMATxAbort(DMA_HandleTypeDef * const hdma);
static void     MMC_DMARxAbort(DMA_HandleTypeDef * const hdma);
#else
static uint32_t MMC_HighSpeed(MMC_HandleTypeDef * const hmmc, FunctionalState state);
static uint32_t MMC_DDR_Mode(MMC_HandleTypeDef * const hmmc, FunctionalState state);
#endif
static HAL_StatusTypeDef MMC_ReadExtCSD(MMC_HandleTypeDef * const hmmc, uint32_t * const pFieldData, uint16_t FieldIndex, uint32_t Timeout);
static uint32_t MMC_PwrClassUpdate(MMC_HandleTypeDef * const hmmc, uint32_t Wide, uint32_t Speed);
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_MMC_Init(MMC_HandleTypeDef * const hmmc)
{
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_MMC_InitCard(MMC_HandleTypeDef * const hmmc)
{
  uint32_t errorstate;
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_MMC_DeInit(MMC_HandleTypeDef * const hmmc)
{
  /* Check the MMC handle allocation */
  if(hmmc == NULL)
  {
...
/**
  ...
  * @retval None
  */
__weak void HAL_MMC_MspInit(MMC_HandleTypeDef * const hmmc)
{
...
/**
  ...
  * @retval None
  */
__weak void HAL_MMC_MspDeInit(MMC_HandleTypeDef * const hmmc)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hmmc);
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_MMC_ReadBlocks(MMC_HandleTypeDef * const hmmc, uint8_t * const pData, uint32_t BlockAdd, uint32_t NumberOfBlocks, uint32_t Timeout)
{
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_MMC_WriteBlocks(MMC_HandleTypeDef * const hmmc, uint8_t const * const pData, uint32_t BlockAdd, uint32_t NumberOfBlocks, uint32_t Timeout)
{
  SDMMC_DataInitTypeDef config;
  uint32_t errorstate;
  uint32_t tickstart = HAL_GetTick();
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_MMC_ReadBlocks_IT(MMC_HandleTypeDef * const hmmc, uint8_t * const pData, uint32_t BlockAdd, uint32_t NumberOfBlocks)
{
  SDMMC_DataInitTypeDef config;
  uint32_t errorstate;
  uint32_t add = BlockAdd;
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_MMC_WriteBlocks_IT(MMC_HandleTypeDef * const hmmc, uint8_t const * const pData, uint32_t BlockAdd, uint32_t NumberOfBlocks)
{
  SDMMC_DataInitTypeDef config;
  uint32_t errorstate;
  uint32_t add = BlockAdd;
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_MMC_ReadBlocks_DMA(MMC_HandleTypeDef * const hmmc, uint8_t * const pData, uint32_t BlockAdd, uint32_t NumberOfBlocks)
{
  SDMMC_DataInitTypeDef config;
  uint32_t errorstate;
  uint32_t add = BlockAdd;
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_MMC_WriteBlocks_DMA(MMC_HandleTypeDef * const hmmc, uint8_t const * const pData, uint32_t BlockAdd, uint32_t NumberOfBlocks)
{
  SDMMC_DataInitTypeDef config;
  uint32_t errorstate;
  uint32_t add = BlockAdd;
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_MMC_Erase(MMC_HandleTypeDef * const hmmc, uint32_t BlockStartAdd, uint32_t BlockEndAdd)
{
  uint32_t errorstate;
...
/**
  ...
  * @retval None
  */
void HAL_MMC_IRQHandler(MMC_HandleTypeDef * const hmmc)
{
  uint32_t errorstate;
...
/**
  ...
  * @retval HAL state
  */
HAL_MMC_StateTypeDef HAL_MMC_GetState(MMC_HandleTypeDef const * const hmmc)
{
  return hmmc->State;
}
...
*/
uint32_t HAL_MMC_GetError(MMC_HandleTypeDef const * const hmmc)
{
...
/**
  ...
  * @retval None
  */
HAL_MMC_WEAK_CALLBACK(void, HAL_MMC_TxCpltCallback)(MMC_HandleTypeDef * const hmmc)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hmmc);
...
/**
  ...
  * @retval None
  */
HAL_MMC_WEAK_CALLBACK(void, HAL_MMC_RxCpltCallback)(MMC_HandleTypeDef * const hmmc)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hmmc);
...
/**
  ...
  * @retval None
  */
HAL_MMC_WEAK_CALLBACK(void, HAL_MMC_ErrorCallback)(MMC_HandleTypeDef * const hmmc)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hmmc);
...
/**
  ...
  * @retval None
  */
HAL_MMC_WEAK_CALLBACK(void, HAL_MMC_AbortCallback)(MMC_HandleTypeDef * const hmmc)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hmmc);
...
/**
  ...
  * @retval status
  */
HAL_StatusTypeDef HAL_MMC_RegisterCallback(MMC_HandleTypeDef * const hmmc, HAL_MMC_CallbackIDTypeDef CallbackId, pMMC_CallbackTypeDef pCallback)
{
...
/**
  ...
  * @retval status
  */
HAL_StatusTypeDef HAL_MMC_UnRegisterCallback(MMC_HandleTypeDef * const hmmc, HAL_MMC_CallbackIDTypeDef CallbackId)
{
  HAL_StatusTypeDef status = HAL_OK;
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_MMC_GetCardCID(MMC_HandleTypeDef const * const hmmc, HAL_MMC_CardCIDTypeDef * const pCID)
{
  pCID->ManufacturerID = (uint8_t)((hmmc->CID[0U] & 0xFF000000UL) >> 24U);

  pCID->OEM_AppliID = (uint16_t)((hmmc->CID[0U] & 0x00FFFF00UL) >> 8U);

  pCID->ProdName1 = (((hmmc->CID[0U] & 0x000000FFUL) << 24U) | ((hmmc->CID[1U] & 0xFFFFFF00UL) >> 8U));

  pCID->ProdName2 = (uint8_t)(hmmc->CID[1U] & 0x000000FFUL);

  pCID->ProdRev = (uint8_t)((hmmc->CID[2U] & 0xFF000000UL) >> 24U);

  pCID->ProdSN = (((hmmc->CID[2U] & 0x00FFFFFFUL) << 8U) | ((hmmc->CID[3U] & 0xFF000000UL) >> 24U));

  pCID->Reserved1 = (uint8_t)((hmmc->CID[3U] & 0x00F00000UL) >> 20U);

  pCID->ManufactDate = (uint16_t)((hmmc->CID[3U] & 0x000FFF00UL) >> 8U);

  pCID->CID_CRC = (uint8_t)((hmmc->CID[3U] & 0x000000FEUL) >> 1U);
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_MMC_GetCardCSD(MMC_HandleTypeDef * const hmmc, HAL_MMC_CardCSDTypeDef * const pCSD)
{
  uint32_t block_nbr = 0;

  pCSD->CSDStruct = (uint8_t)((hmmc->CSD[0U] & 0xC0000000UL) >> 30U);

  pCSD->SysSpecVersion = (uint8_t)((hmmc->CSD[0U] & 0x3C000000UL) >> 26U);

  pCSD->Reserved1 = (uint8_t)((hmmc->CSD[0U] & 0x03000000UL) >> 24U);

  pCSD->TAAC = (uint8_t)((hmmc->CSD[0U] & 0x00FF0000UL) >> 16U);

  pCSD->NSAC = (uint8_t)((hmmc->CSD[0U] & 0x0000FF00UL) >> 8U);

  pCSD->MaxBusClkFrec = (uint8_t)(hmmc->CSD[0U] & 0x000000FFUL);

  pCSD->CardComdClasses = (uint16_t)((hmmc->CSD[1U] & 0xFFF00000UL) >> 20U);

  pCSD->RdBlockLen = (uint8_t)((hmmc->CSD[1U] & 0x000F0000UL) >> 16U);

  pCSD->PartBlockRead   = (uint8_t)((hmmc->CSD[1U] & 0x00008000UL) >> 15U);

  pCSD->WrBlockMisalign = (uint8_t)((hmmc->CSD[1U] & 0x00004000UL) >> 14U);

  pCSD->RdBlockMisalign = (uint8_t)((hmmc->CSD[1U] & 0x00002000UL) >> 13U);

  pCSD->DSRImpl = (uint8_t)((hmmc->CSD[1U] & 0x00001000UL) >> 12U);

  pCSD->Reserved2 = 0U; /*!< Reserved */

  if(MMC_ReadExtCSD(hmmc, &block_nbr, 212, 0x0FFFFFFFUL) != HAL_OK) /* Field SEC_COUNT [215:212] */
  {
    return HAL_ERROR;
  }
  ...
  {
    pCSD->DeviceSize = (((hmmc->CSD[1U] & 0x000003FFUL) << 2U) | ((hmmc->CSD[2U] & 0xC0000000UL) >> 30U));

    pCSD->MaxRdCurrentVDDMin = (uint8_t)((hmmc->CSD[2U] & 0x38000000UL) >> 27U);

    pCSD->MaxRdCurrentVDDMax = (uint8_t)((hmmc->CSD[2U] & 0x07000000UL) >> 24U);

    pCSD->MaxWrCurrentVDDMin = (uint8_t)((hmmc->CSD[2U] & 0x00E00000UL) >> 21U);

    pCSD->MaxWrCurrentVDDMax = (uint8_t)((hmmc->CSD[2U] & 0x001C0000UL) >> 18U);

    pCSD->DeviceSizeMul = (uint8_t)((hmmc->CSD[2U] & 0x00038000UL) >> 15U);
  ...
  pCSD->EraseGrSize = (uint8_t)((hmmc->CSD[2U] & 0x00004000UL) >> 14U);

  pCSD->EraseGrMul = (uint8_t)((hmmc->CSD[2U] & 0x00003F80UL) >> 7U);

  pCSD->WrProtectGrSize = (uint8_t)(hmmc->CSD[2U] & 0x0000007FUL);

  pCSD->WrProtectGrEnable = (uint8_t)((hmmc->CSD[3U] & 0x80000000UL) >> 31U);

  pCSD->ManDeflECC = (uint8_t)((hmmc->CSD[3U] & 0x60000000UL) >> 29U);

  pCSD->WrSpeedFact = (uint8_t)((hmmc->CSD[3U] & 0x1C000000UL) >> 26U);

  pCSD->MaxWrBlockLen= (uint8_t)((hmmc->CSD[3U] & 0x03C00000UL) >> 22U);

  pCSD->WriteBlockPaPartial = (uint8_t)((hmmc->CSD[3U] & 0x00200000UL) >> 21U);

  pCSD->Reserved3 = 0;

  pCSD->ContentProtectAppli = (uint8_t)((hmmc->CSD[3U] & 0x00010000UL) >> 16U);

  pCSD->FileFormatGroup = (uint8_t)((hmmc->CSD[3U] & 0x00008000UL) >> 15U);

  pCSD->CopyFlag = (uint8_t)((hmmc->CSD[3U] & 0x00004000UL) >> 14U);

  pCSD->PermWrProtect = (uint8_t)((hmmc->CSD[3U] & 0x00002000UL) >> 13U);

  pCSD->TempWrProtect = (uint8_t)((hmmc->CSD[3U] & 0x00001000UL) >> 12U);

  pCSD->FileFormat = (uint8_t)((hmmc->CSD[3U] & 0x00000C00UL) >> 10U);

  pCSD->ECC= (uint8_t)((hmmc->CSD[3U] & 0x00000300UL) >> 8U);

  pCSD->CSD_CRC = (uint8_t)((hmmc->CSD[3U] & 0x000000FEUL) >> 1U);
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_MMC_GetCardInfo(MMC_HandleTypeDef const * const hmmc, HAL_MMC_CardInfoTypeDef * const pCardInfo)
{
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_MMC_GetCardExtCSD(MMC_HandleTypeDef * const hmmc, uint32_t * const pExtCSD, uint32_t Timeout)
{
  SDMMC_DataInitTypeDef config;
  uint32_t errorstate;
  uint32_t tickstart = HAL_GetTick();
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_MMC_ConfigWideBusOperation(MMC_HandleTypeDef * const hmmc, uint32_t WideMode)
{
  uint32_t count;
  ...
  if(errorstate == HAL_MMC_ERROR_NONE)
  {
    ...
    {
      errorstate = SDMMC_CmdSwitch(hmmc->Instance, 0x03B70200UL);
    }
    else if(WideMode == SDMMC_BUS_WIDE_4B)
    {
      errorstate = SDMMC_CmdSwitch(hmmc->Instance, 0x03B70100UL);
    }
    else if(WideMode == SDMMC_BUS_WIDE_1B)
    {
      errorstate = SDMMC_CmdSwitch(hmmc->Instance, 0x03B70000UL);
    }
    else
    {
...
HAL_StatusTypeDef HAL_MMC_ConfigSpeedBusOperation(MMC_HandleTypeDef * const hmmc, uint32_t SpeedMode)
{
  uint32_t tickstart;
  HAL_StatusTypeDef status = HAL_OK;
  ...
  device_type = (hmmc->Ext_CSD[49U] & 0x000000FFUL);
...
/**
  ...
  * @retval Card state
  */
HAL_MMC_CardStateTypeDef HAL_MMC_GetCardState(MMC_HandleTypeDef * const hmmc)
{
  uint32_t cardstate;
  uint32_t errorstate;
  uint32_t resp1 = 0U;
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_MMC_Abort(MMC_HandleTypeDef * const hmmc)
{
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_MMC_Abort_IT(MMC_HandleTypeDef * const hmmc)
{
  HAL_MMC_CardStateTypeDef CardState;
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_MMC_EraseSequence(MMC_HandleTypeDef * const hmmc, uint32_t EraseType, uint32_t BlockStartAdd, uint32_t BlockEndAdd)
{
  uint32_t errorstate;
  uint32_t start_add = BlockStartAdd;
  uint32_t end_add = BlockEndAdd;
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_MMC_Sanitize(MMC_HandleTypeDef * const hmmc)
{
  ...
  if(hmmc->State == HAL_MMC_STATE_READY)
  {
    ...
    /* Index : 165 - Value : 0x01U */
    errorstate = SDMMC_CmdSwitch(hmmc->Instance, 0x03A50100UL);
    if(errorstate == HAL_MMC_ERROR_NONE)
    {
      /* Wait that the device is ready by checking the D0 line */
      while((!__HAL_MMC_GET_FLAG(hmmc, SDMMC_FLAG_BUSYD0END)) && (errorstate == HAL_MMC_ERROR_NONE))
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_MMC_ConfigSecRemovalType(MMC_HandleTypeDef * const hmmc, uint32_t SRTMode)
{
  ...
  if(hmmc->State == HAL_MMC_STATE_READY)
  {
    ...
    if(HAL_MMC_GetSupportedSecRemovalType(hmmc, &srt) == HAL_OK)
    {
      ...
      if((SRTMode & srt) != 0U)
      {
        ...
        errorstate = SDMMC_CmdSwitch(hmmc->Instance, (0x03100000UL | (srt << 8U)));
        if(errorstate == HAL_MMC_ERROR_NONE)
        {
          /* While card is not ready for data and trial number for sending CMD13 is not exceeded */
          count = SDMMC_MAX_TRIAL;
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_MMC_GetSupportedSecRemovalType(MMC_HandleTypeDef * const hmmc, uint32_t * const SupportedSRT)
{
  /* Check the state of the driver */
  if(hmmc->State == HAL_MMC_STATE_READY)
  {
    ...
    *SupportedSRT = (hmmc->Ext_CSD[4U] & 0x0000000FUL); /* Bits [3:0] of field 16 */
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_MMC_SleepDevice(MMC_HandleTypeDef * const hmmc)
{
  ...
  if(hmmc->State == HAL_MMC_STATE_READY)
  {
    ...
    /* Set the power-off notification to powered-on : Ext_CSD[34U] = 1 */
    errorstate = SDMMC_CmdSwitch(hmmc->Instance, (0x03220100UL));
    if (errorstate == HAL_MMC_ERROR_NONE)
    {
      /* While card is not ready for data and trial number for sending CMD13 is not exceeded */
      count = SDMMC_MAX_TRIAL;
      ...
      else if (errorstate == HAL_MMC_ERROR_NONE)
      {
        ...
        {
          /* Set the power-off notification to sleep notification : Ext_CSD[34U] = 4 */
          errorstate = SDMMC_CmdSwitch(hmmc->Instance, (0x03220400UL));
          if (errorstate == HAL_MMC_ERROR_NONE)
          {
            /* Field SLEEP_NOTIFICATION_TIME [216U] */
            sleep_timeout = ((hmmc->Ext_CSD[(MMC_EXT_CSD_SLEEP_NOTIFICATION_TIME_INDEX/4)] >> MMC_EXT_CSD_SLEEP_NOTIFICATION_TIME_POS) & 0x000000FFUL);

            /* Sleep/Awake Timeout = 10µs * 2^SLEEP_NOTIFICATION_TIME, max value of SLEEP_NOTIFICATION_TIME is 0x17U */
            ...
            if (errorstate == HAL_MMC_ERROR_NONE)
            {
              ...
              else if (errorstate == HAL_MMC_ERROR_NONE)
              {
                ...
                else
                {
                  ...
                  /* Field S_A_TIEMOUT [217U] */
                  sleep_timeout = ((hmmc->Ext_CSD[(MMC_EXT_CSD_S_A_TIMEOUT_INDEX/4)] >> MMC_EXT_CSD_S_A_TIMEOUT_POS) & 0x000000FFUL);

                  /* Sleep/Awake Timeout = 100ns * 2^S_A_TIMEOUT, max value of S_A_TIMEOUT is 0x17U */
                  /* In HAL, the tick interrupt occurs each ms */
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_MMC_AwakeDevice(MMC_HandleTypeDef * const hmmc)
{
  uint32_t errorstate, sleep_timeout, timeout, count, response = 0U;
  uint32_t tickstart = HAL_GetTick();
...
static void MMC_DMATransmitCplt(DMA_HandleTypeDef * const hdma)
{
  MMC_HandleTypeDef * const hmmc = (MMC_HandleTypeDef * )(hdma->Parent);
...
static uint32_t MMC_PowerON(MMC_HandleTypeDef * const hmmc)
{
  ...
  if (((response & (0xFF000000UL)) >> 24U) == 0xC0U)
  {
...
static uint32_t MMC_HighSpeed(MMC_HandleTypeDef * const hmmc, FunctionalState state)
{
  ...
  if (((hmmc->Instance->CLKCR & SDMMC_CLKCR_BUSSPEED) != 0U) && (state == DISABLE))
  {
    ...
    {
      /* Index : 185 - Value : 0 */
      errorstate = SDMMC_CmdSwitch(hmmc->Instance, 0x03B90000UL);
    }
  }
  ...
  if (((hmmc->Instance->CLKCR & SDMMC_CLKCR_BUSSPEED) == 0U) && (state != DISABLE))
  {
    ...
    {
      /* Index : 185 - Value : 1 */
      errorstate = SDMMC_CmdSwitch(hmmc->Instance, 0x03B90100UL);
    }
  }
...
static uint32_t MMC_DDR_Mode(MMC_HandleTypeDef * const hmmc, FunctionalState state)
{
  ...
  if (((hmmc->Instance->CLKCR & SDMMC_CLKCR_DDR) == 0U) && (state != DISABLE))
  {
    if ((hmmc->Instance->CLKCR & SDMMC_CLKCR_WIDBUS_0) != 0U)
    {
      ...
      {
        /* Index : 183 - Value : 5 */
        errorstate = SDMMC_CmdSwitch(hmmc->Instance, 0x03B70500UL);
      }
    }
    else
    {
      ...
      {
        /* Index : 183 - Value : 6 */
        errorstate = SDMMC_CmdSwitch(hmmc->Instance, 0x03B70600UL);
      }
    }
  }
...
static uint32_t MMC_PwrClassUpdate(MMC_HandleTypeDef * const hmmc, uint32_t Wide, uint32_t Speed)
{
  ...
  if((Wide == SDMMC_BUS_WIDE_8B) || (Wide == SDMMC_BUS_WIDE_4B))
  {
    ...
    if(MMC_ReadExtCSD(hmmc, &power_class, 187, SDMMC_DATATIMEOUT) != HAL_OK) /* Field POWER_CLASS [187U] */
    {
      errorstate = SDMMC_ERROR_GENERAL_UNKNOWN_ERR;
    }
    else
    {
      power_class = ((power_class >> 24U) & 0x000000FFUL);
    }
    ...
    if (Speed == SDMMC_SPEED_MODE_DDR)
    {
      ...
      supported_pwr_class = ((hmmc->Ext_CSD[(MMC_EXT_CSD_PWR_CL_DDR_52_INDEX/4)] >> MMC_EXT_CSD_PWR_CL_DDR_52_POS) & 0x000000FFUL);
    }
    else if (Speed == SDMMC_SPEED_MODE_HIGH)
    {
      /* Field PWR_CL_52_xxx [200 or 202] */
      supported_pwr_class = ((hmmc->Ext_CSD[(MMC_EXT_CSD_PWR_CL_52_INDEX/4)] >> MMC_EXT_CSD_PWR_CL_52_POS) & 0x000000FFUL);
    }
    else
...
#endif
    {
      ...
      supported_pwr_class = ((hmmc->Ext_CSD[(MMC_EXT_CSD_PWR_CL_26_INDEX/4)] >> MMC_EXT_CSD_PWR_CL_26_POS) & 0x000000FFUL);
    }
    ...
    if(errorstate == HAL_MMC_ERROR_NONE)
    {
      ...
      if ((power_class & 0x0FU) != (supported_pwr_class & 0x0FU))
      {
        ...
        errorstate = SDMMC_CmdSwitch(hmmc->Instance, (0x03BB0000UL | ((supported_pwr_class & 0x0FU) << 8U)));
