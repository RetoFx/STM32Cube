...
typedef enum
{
  HAL_SWPMI_STATE_RESET             = 0x00U,    /*!< Peripheral Reset state                             */
  HAL_SWPMI_STATE_READY             = 0x01U,    /*!< Peripheral Initialized and ready for use           */
  HAL_SWPMI_STATE_BUSY              = 0x02U,    /*!< an internal process is ongoing                     */
  HAL_SWPMI_STATE_BUSY_TX           = 0x12U,    /*!< Data Transmission process is ongoing               */
  HAL_SWPMI_STATE_BUSY_RX           = 0x22U,    /*!< Data Reception process is ongoing                  */
  HAL_SWPMI_STATE_BUSY_TX_RX        = 0x32U,    /*!< Data Transmission and Reception process is ongoing */
  HAL_SWPMI_STATE_TIMEOUT           = 0x03U,    /*!< Timeout state                                      */
  HAL_SWPMI_STATE_ERROR             = 0x04U     /*!< Error                                              */
} HAL_SWPMI_StateTypeDef;

/**
  * @brief  SWPMI handle Structure definition
  */
#if (USE_HAL_SWPMI_REGISTER_CALLBACKS == 1U)
...
#endif /* USE_HAL_SWPMI_REGISTER_CALLBACKS */
{
  ...
  uint32_t const                 *pTxBuffPtr;   /*!< Pointer to SWPMI Tx transfer Buffer  */
...
#if (USE_HAL_SWPMI_REGISTER_CALLBACKS == 1U)
  void (*RxCpltCallback)(struct __SWPMI_HandleTypeDef * const hswpmi);      /*!< SWPMI receive complete callback */
  void (*RxHalfCpltCallback)(struct __SWPMI_HandleTypeDef * const hswpmi);  /*!< SWPMI receive half complete callback */
  void (*TxCpltCallback)(struct __SWPMI_HandleTypeDef * const hswpmi);      /*!< SWPMI transmit complete callback */
  void (*TxHalfCpltCallback)(struct __SWPMI_HandleTypeDef * const hswpmi);  /*!< SWPMI transmit half complete callback */
  void (*ErrorCallback)(struct __SWPMI_HandleTypeDef * const hswpmi);       /*!< SWPMI error callback */
  void (*MspInitCallback)(struct __SWPMI_HandleTypeDef * const hswpmi);     /*!< SWPMI MSP init callback */
  void (*MspDeInitCallback)(struct __SWPMI_HandleTypeDef * const hswpmi);   /*!< SWPMI MSP de-init callback */
#endif
...
/**
  ...
  */
typedef void (*pSWPMI_CallbackTypeDef)(SWPMI_HandleTypeDef * const hswpmi);
#endif
...
/**
  ...
  * @{
  */
#define HAL_SWPMI_ERROR_NONE                  ((uint32_t)0x00000000UL) /*!< No error              */
#define HAL_SWPMI_ERROR_CRC                   ((uint32_t)0x00000004UL) /*!< frame error           */
#define HAL_SWPMI_ERROR_OVR                   ((uint32_t)0x00000008UL) /*!< Overrun error         */
#define HAL_SWPMI_ERROR_UDR                   ((uint32_t)0x0000000CUL) /*!< Underrun error        */
#define HAL_SWPMI_ERROR_DMA                   ((uint32_t)0x00000010UL) /*!< DMA transfer error    */
#define HAL_SWPMI_ERROR_TIMEOUT               ((uint32_t)0x00000020UL) /*!< Transfer timeout      */
#define HAL_SWPMI_ERROR_TXBEF_TIMEOUT         ((uint32_t)0x00000040UL) /*!< End Tx buffer timeout */
#if (USE_HAL_SWPMI_REGISTER_CALLBACKS == 1U)
#define HAL_SWPMI_ERROR_INVALID_CALLBACK      ((uint32_t)0x00000100UL) /*!< Invalid callback error */
#endif
/**
  * @}
  */
...
/** @defgroup SWPMI_Voltage_Class SWPMI Voltage Class
  * @{
  */
#define SWPMI_VOLTAGE_CLASS_C                ((uint32_t)0x00000000UL)
...
/** @defgroup SWPMI_Tx_Buffering_Mode SWPMI Tx Buffering Mode
  * @{
  */
#define SWPMI_TX_NO_SOFTWAREBUFFER           ((uint32_t)0x00000000UL)
#define SWPMI_TX_SINGLE_SOFTWAREBUFFER       ((uint32_t)0x00000000UL)
...
/** @defgroup SWPMI_Rx_Buffering_Mode SWPMI Rx Buffering Mode
  * @{
  */
#define SWPMI_RX_NO_SOFTWAREBUFFER           ((uint32_t)0x00000000UL)
#define SWPMI_RX_SINGLE_SOFTWAREBUFFER       ((uint32_t)0x00000000UL)
...
HAL_StatusTypeDef HAL_SWPMI_Init(SWPMI_HandleTypeDef * const hswpmi);
HAL_StatusTypeDef HAL_SWPMI_DeInit(SWPMI_HandleTypeDef * const hswpmi);
void              HAL_SWPMI_MspInit(SWPMI_HandleTypeDef * const hswpmi);
void              HAL_SWPMI_MspDeInit(SWPMI_HandleTypeDef * const hswpmi);

#if (USE_HAL_SWPMI_REGISTER_CALLBACKS == 1U)
/* SWPMI callbacks register/unregister functions ********************************/
HAL_StatusTypeDef HAL_SWPMI_RegisterCallback(SWPMI_HandleTypeDef        * const hswpmi,
                                             HAL_SWPMI_CallbackIDTypeDef CallbackID,
                                             pSWPMI_CallbackTypeDef      pCallback);
HAL_StatusTypeDef HAL_SWPMI_UnRegisterCallback(SWPMI_HandleTypeDef        * const hswpmi,
                                               HAL_SWPMI_CallbackIDTypeDef CallbackID);
#endif

/* IO operation functions *****************************************************/
HAL_StatusTypeDef HAL_SWPMI_Transmit(SWPMI_HandleTypeDef * const hswpmi, uint32_t const * const pData, uint16_t Size, uint32_t Timeout);
HAL_StatusTypeDef HAL_SWPMI_Receive(SWPMI_HandleTypeDef * const hswpmi, uint32_t * const pData, uint16_t Size, uint32_t Timeout);
HAL_StatusTypeDef HAL_SWPMI_Transmit_IT(SWPMI_HandleTypeDef * const hswpmi, uint32_t const * const pData, uint16_t Size);
HAL_StatusTypeDef HAL_SWPMI_Receive_IT(SWPMI_HandleTypeDef * const hswpmi, uint32_t * const pData, uint16_t Size);
HAL_StatusTypeDef HAL_SWPMI_Transmit_DMA(SWPMI_HandleTypeDef * const hswpmi, uint32_t const * const pData, uint16_t Size);
HAL_StatusTypeDef HAL_SWPMI_Receive_DMA(SWPMI_HandleTypeDef * const hswpmi, uint32_t * const pData, uint16_t Size);
HAL_StatusTypeDef HAL_SWPMI_DMAStop(SWPMI_HandleTypeDef * const hswpmi);
HAL_StatusTypeDef HAL_SWPMI_EnableLoopback(SWPMI_HandleTypeDef * const hswpmi);
HAL_StatusTypeDef HAL_SWPMI_DisableLoopback(SWPMI_HandleTypeDef * const hswpmi);
void              HAL_SWPMI_IRQHandler(SWPMI_HandleTypeDef * const hswpmi);
void              HAL_SWPMI_TxCpltCallback(SWPMI_HandleTypeDef * const hswpmi);
void              HAL_SWPMI_TxHalfCpltCallback(SWPMI_HandleTypeDef * const hswpmi);
void              HAL_SWPMI_RxCpltCallback(SWPMI_HandleTypeDef * const hswpmi);
void              HAL_SWPMI_RxHalfCpltCallback(SWPMI_HandleTypeDef * const hswpmi);
void              HAL_SWPMI_ErrorCallback(SWPMI_HandleTypeDef * const hswpmi);

/* Peripheral Control and State functions  ************************************/
HAL_SWPMI_StateTypeDef HAL_SWPMI_GetState(SWPMI_HandleTypeDef const * const hswpmi);
uint32_t               HAL_SWPMI_GetError(SWPMI_HandleTypeDef const * const hswpmi);
