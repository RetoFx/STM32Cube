...
static void LTDC_SetConfig(LTDC_HandleTypeDef * const hltdc, LTDC_LayerCfgTypeDef const * const pLayerCfg, uint32_t LayerIdx);
/* Private functions ---------------------------------------------------------*/
...
/**
  ...
  * @retval None
  */
__weak void HAL_LTDC_MspInit(LTDC_HandleTypeDef * const hltdc)
{
...
/**
  ...
  * @retval None
  */
__weak void HAL_LTDC_MspDeInit(LTDC_HandleTypeDef * const hltdc)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hltdc);
...
/**
  ...
  * @retval None
  */
HAL_LTDC_WEAK_CALLBACK(void, HAL_LTDC_ErrorCallback)(LTDC_HandleTypeDef * const hltdc)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hltdc);
...
/**
  ...
  * @retval None
  */
HAL_LTDC_WEAK_CALLBACK(void, HAL_LTDC_LineEventCallback)(LTDC_HandleTypeDef * const hltdc)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hltdc);
...
/**
  ...
  * @retval None
  */
HAL_LTDC_WEAK_CALLBACK(void, HAL_LTDC_ReloadEventCallback)(LTDC_HandleTypeDef * const hltdc)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hltdc);
                                      ...
                                      uint32_t LayerIdx)
{
  uint32_t tmp;
  uint32_t counter;
  uint32_t const * pcolorlut = pCLUT;
  /* Check the parameters */
  assert_param(IS_LTDC_LAYER(LayerIdx));
  ...
  for (counter = 0U; (counter < CLUTSize); counter++)
  {
    if (hltdc->LayerCfg[LayerIdx].PixelFormat == LTDC_PIXEL_FORMAT_AL44)
    {
      tmp  = (((counter + (16U * counter)) << 24U) | ((uint32_t)(*pcolorlut) & 0xFFU) |
              ((uint32_t)(*pcolorlut) & 0xFF00U) | ((uint32_t)(*pcolorlut) & 0xFF0000UL));
    }
    else
    {
