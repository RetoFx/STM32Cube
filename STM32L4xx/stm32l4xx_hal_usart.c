...
static void USART_EndTransfer(USART_HandleTypeDef * const husart);
...
static HAL_StatusTypeDef USART_SetConfig(USART_HandleTypeDef * const husart);
static HAL_StatusTypeDef USART_CheckIdleState(USART_HandleTypeDef * const husart);
static void USART_TxISR_8BIT(USART_HandleTypeDef * const husart);
static void USART_TxISR_16BIT(USART_HandleTypeDef * const husart);
#if defined(USART_CR1_FIFOEN)
static void USART_TxISR_8BIT_FIFOEN(USART_HandleTypeDef * const husart);
static void USART_TxISR_16BIT_FIFOEN(USART_HandleTypeDef * const husart);
#endif /* USART_CR1_FIFOEN */
static void USART_EndTransmit_IT(USART_HandleTypeDef * const husart);
static void USART_RxISR_8BIT(USART_HandleTypeDef * const husart);
static void USART_RxISR_16BIT(USART_HandleTypeDef * const husart);
#if defined(USART_CR1_FIFOEN)
static void USART_RxISR_8BIT_FIFOEN(USART_HandleTypeDef * const husart);
static void USART_RxISR_16BIT_FIFOEN(USART_HandleTypeDef * const husart);
#endif /* USART_CR1_FIFOEN */
...
HAL_StatusTypeDef HAL_USART_Transmit_DMA(USART_HandleTypeDef * const husart, uint8_t const * const pTxData, uint16_t Size)
{
  ...
  if (husart->State == HAL_USART_STATE_READY)
  {
    ...
    if (husart->hdmatx != NULL)
    {
      ...
      status = HAL_DMA_Start_IT(husart->hdmatx, *(uint32_t const *)tmp, (uint32_t)&husart->Instance->TDR, Size);
    }
                                                ...
                                                uint16_t Size)
{
  ...
  if (husart->State == HAL_USART_STATE_READY)
  {
    ...
    if ((husart->hdmarx != NULL) && (husart->hdmatx != NULL))
    {
      ...
      status = HAL_DMA_Start_IT(husart->hdmarx, (uint32_t)&husart->Instance->RDR, *(uint32_t const *)tmp, Size);
...
static void USART_DMATransmitCplt(DMA_HandleTypeDef * const hdma)
{
  USART_HandleTypeDef * const husart = (USART_HandleTypeDef *)(hdma->Parent);
...
static HAL_StatusTypeDef USART_SetConfig(USART_HandleTypeDef * const husart)
{
  ...
  uint16_t brrtemp;
  uint32_t usartdiv                    = 0x00000000UL;
  uint32_t pclk;
