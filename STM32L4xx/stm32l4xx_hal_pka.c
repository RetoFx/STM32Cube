...
/** @defgroup PKA_Private_Functions PKA Private Functions
  * @{
  */
uint32_t PKA_GetMode(PKA_HandleTypeDef const * const hpka);
HAL_StatusTypeDef PKA_PollEndOfOperation(PKA_HandleTypeDef const * const hpka, uint32_t Timeout, uint32_t Tickstart);
uint32_t PKA_CheckError(PKA_HandleTypeDef const * const hpka, uint32_t mode);
...
void PKA_Memcpy_u32_to_u8(uint8_t dst[], __IO uint32_t const src[], size_t n);
void PKA_Memcpy_u8_to_u32(__IO uint32_t dst[], uint8_t const src[], size_t n);
void PKA_Memcpy_u32_to_u32(__IO uint32_t dst[], __IO uint32_t const src[], size_t n);
HAL_StatusTypeDef PKA_Process(PKA_HandleTypeDef * const hpka, uint32_t mode, uint32_t Timeout);
HAL_StatusTypeDef PKA_Process_IT(PKA_HandleTypeDef * const hpka, uint32_t mode);
void PKA_ModExp_Set(PKA_HandleTypeDef * const hpka, PKA_ModExpInTypeDef const * const in);
void PKA_ModExpFastMode_Set(PKA_HandleTypeDef * const hpka, PKA_ModExpFastModeInTypeDef const * const in);
void PKA_ECDSASign_Set(PKA_HandleTypeDef * const hpka, PKA_ECDSASignInTypeDef const * const in);
void PKA_ECDSAVerif_Set(PKA_HandleTypeDef * const hpka, PKA_ECDSAVerifInTypeDef const * const in);
void PKA_RSACRTExp_Set(PKA_HandleTypeDef * const hpka, PKA_RSACRTExpInTypeDef const * const in);
void PKA_PointCheck_Set(PKA_HandleTypeDef * const hpka, PKA_PointCheckInTypeDef const * const in);
void PKA_ECCMul_Set(PKA_HandleTypeDef * const hpka, PKA_ECCMulInTypeDef const * const in);
void PKA_ECCMulFastMode_Set(PKA_HandleTypeDef * const hpka, PKA_ECCMulFastModeInTypeDef const * const in);
void PKA_ModRed_Set(PKA_HandleTypeDef * const hpka, PKA_ModRedInTypeDef const * const in);
void PKA_ModInv_Set(PKA_HandleTypeDef * const hpka, PKA_ModInvInTypeDef const * const in);
void PKA_MontgomeryParam_Set(PKA_HandleTypeDef * const hpka, uint32_t const size, uint8_t const * const pOp1);
void PKA_ARI_Set(PKA_HandleTypeDef * const hpka, uint32_t const size, uint32_t const * const pOp1, uint32_t const * const pOp2,
                 uint8_t const * const pOp3);
/**
  * @}
  */
...
/**
  ...
  * @retval None
  */
__weak void HAL_PKA_MspInit(PKA_HandleTypeDef * const hpka)
{
...
/**
  ...
  * @retval None
  */
__weak void HAL_PKA_MspDeInit(PKA_HandleTypeDef * const hpka)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hpka);
...
void HAL_PKA_Arithmetic_GetResult(PKA_HandleTypeDef const * const hpka, uint32_t * const pRes)
{
  ...
  switch (mode)
  {
    ...
    case PKA_MODE_MONTGOMERY_MUL:
      size = hpka->Instance->RAM[1U] / 32UL;
      break;
    ...
    case PKA_MODE_ARITHMETIC_MUL:
      size = hpka->Instance->RAM[1U] / 32UL * 2UL;
      break;
    default:
      break;
  }
...
/**
  ...
  * @retval None
  */
__weak void HAL_PKA_OperationCpltCallback(PKA_HandleTypeDef * const hpka)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hpka);
...
/**
  ...
  * @retval None
  */
__weak void HAL_PKA_ErrorCallback(PKA_HandleTypeDef * const hpka)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hpka);
  ...
  if (dst != NULL)
  {
    if (src != NULL)
    {
      ...
      for (; index_uint32_t < (n / 4UL); index_uint32_t++)
      {
        ...
        dst[index_uint8_t + 3UL] = (uint8_t)((src[index_uint32_t] & 0x000000FFUL));
        dst[index_uint8_t + 2UL] = (uint8_t)((src[index_uint32_t] & 0x0000FF00UL) >> 8UL);
        dst[index_uint8_t + 1UL] = (uint8_t)((src[index_uint32_t] & 0x00FF0000UL) >> 16UL);
        dst[index_uint8_t + 0UL] = (uint8_t)((src[index_uint32_t] & 0xFF000000UL) >> 24UL);
      }

      /* Manage the buffers not aligned on uint32_t */
      if ((n % 4UL) == 1UL)
      {
        dst[0UL] = (uint8_t)((src[index_uint32_t] & 0x000000FFUL));
      }
      else if ((n % 4UL) == 2UL)
      {
        dst[1UL] = (uint8_t)((src[index_uint32_t] & 0x000000FFUL));
        dst[0UL] = (uint8_t)((src[index_uint32_t] & 0x0000FF00UL) >> 8UL);
      }
      else if ((n % 4UL) == 3UL)
      {
        dst[2UL] = (uint8_t)((src[index_uint32_t] & 0x000000FFUL));
        dst[1UL] = (uint8_t)((src[index_uint32_t] & 0x0000FF00UL) >> 8UL);
        dst[0UL] = (uint8_t)((src[index_uint32_t] & 0x00FF0000UL) >> 16UL);
      }
      else
      {
