...
==============================================================================
    [..]
     (#)Initialize the HRTIM low level resources by implementing the
        ...
        (##)Enable the HRTIM clock source using __HAL_RCC_HRTIMx_CLK_ENABLE()
...
/** @defgroup HRTIM_Private_Variables HRTIM Private Variables
  * @{
  */
static uint32_t const TimerIdxToTimerId[] =
{
...
/** @defgroup HRTIM_Private_Functions HRTIM Private Functions
  * @{
  */
static void HRTIM_MasterBase_Config(HRTIM_HandleTypeDef * const hhrtim,
                                    HRTIM_TimeBaseCfgTypeDef const * const pTimeBaseCfg);

static void HRTIM_TimingUnitBase_Config(HRTIM_HandleTypeDef * const hhrtim,
                                        uint32_t TimerIdx,
                                        HRTIM_TimeBaseCfgTypeDef const * const pTimeBaseCfg);

static void HRTIM_MasterWaveform_Config(HRTIM_HandleTypeDef * const hhrtim,
                                        HRTIM_TimerCfgTypeDef const * const pTimerCfg);

static void HRTIM_TimingUnitWaveform_Config(HRTIM_HandleTypeDef * const hhrtim,
                                            uint32_t TimerIdx,
                                            HRTIM_TimerCfgTypeDef const * const pTimerCfg);

static void HRTIM_TimingUnitWaveform_Control(HRTIM_HandleTypeDef * const hhrtim,
                                             uint32_t TimerIdx,
                                             HRTIM_TimerCtlTypeDef const * const pTimerCtl);

static void  HRTIM_TimingUnitRollOver_Config(HRTIM_HandleTypeDef * const hhrtim,
                                             uint32_t TimerIdx,
                                             uint32_t pRollOverMode);


static void HRTIM_CaptureUnitConfig(HRTIM_HandleTypeDef * const hhrtim,
                                    uint32_t TimerIdx,
                                    uint32_t CaptureUnit,
                                    uint32_t Event);

static void HRTIM_OutputConfig(HRTIM_HandleTypeDef * const hhrtim,
                                uint32_t TimerIdx,
                                uint32_t Output,
                                HRTIM_OutputCfgTypeDef const * const pOutputCfg);

static void HRTIM_EventConfig(HRTIM_HandleTypeDef * const hhrtim,
                              uint32_t Event,
                              HRTIM_EventCfgTypeDef const * const pEventCfg);

static void HRTIM_TIM_ResetConfig(HRTIM_HandleTypeDef * const hhrtim,
                                  uint32_t TimerIdx,
                                  uint32_t Event);

static uint32_t HRTIM_GetITFromOCMode(HRTIM_HandleTypeDef const * const hhrtim,
                                      uint32_t TimerIdx,
                                      uint32_t OCChannel);

static uint32_t HRTIM_GetDMAFromOCMode(HRTIM_HandleTypeDef const * const hhrtim,
                                       uint32_t TimerIdx,
                                       uint32_t OCChannel);

static DMA_HandleTypeDef * HRTIM_GetDMAHandleFromTimerIdx(HRTIM_HandleTypeDef const * const hhrtim,
                                                          uint32_t TimerIdx);

static uint32_t GetTimerIdxFromDMAHandle(HRTIM_HandleTypeDef const * const hhrtim,
                                         DMA_HandleTypeDef const * const hdma);

static void HRTIM_ForceRegistersUpdate(HRTIM_HandleTypeDef * const hhrtim,
                                      uint32_t TimerIdx);

static void HRTIM_HRTIM_ISR(HRTIM_HandleTypeDef * const hhrtim);

static void HRTIM_Master_ISR(HRTIM_HandleTypeDef * const hhrtim);

static void HRTIM_Timer_ISR(HRTIM_HandleTypeDef * const hhrtim,
                                       uint32_t TimerIdx);

static void HRTIM_DMAMasterCplt(DMA_HandleTypeDef * const hdma);

static void HRTIM_DMATimerxCplt(DMA_HandleTypeDef * const hdma);

static void HRTIM_DMAError(DMA_HandleTypeDef * const hdma);

static void HRTIM_BurstDMACplt(DMA_HandleTypeDef * const hdma);
/**
  * @}
  */
...
/**
  ...
  * @retval None
  */
__weak void HAL_HRTIM_MspInit(HRTIM_HandleTypeDef * const hhrtim)
{
...
/**
  ...
  * @retval None
  */
__weak void HAL_HRTIM_MspDeInit(HRTIM_HandleTypeDef * const hhrtim)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hhrtim);
                                        ...
                                        HRTIM_FaultCfgTypeDef const * const pFaultCfg)
{
  ...
  source1 = ((pFaultCfg->Source & 2U) >> 1U);
                                                      ...
                                                      HRTIM_ExternalEventCfgTypeDef const * const pTimerExternalEventCfg)
{
  ...
  if ((EventCounter & HRTIM_EVENTCOUNTER_A) != 0U)
  {
   ...
   { /* reset External EventCounter A */
     WRITE_REG(hhrtim->Instance->sTimerxRegs[TimerIdx].EEFxR3, 0xFFFF0000UL);
   }
   else
   {
  ...
  if ((EventCounter & HRTIM_EVENTCOUNTER_B) != 0U)
  {
   ...
   { /* reset External EventCounter B */
     WRITE_REG(hhrtim->Instance->sTimerxRegs[TimerIdx].EEFxR3, 0x0000FFFFUL);
   }
   else
   {
...
{
  uint32_t Trigger;
  uint32_t TimerF_Trigger = (uint32_t)(pCaptureCfg->Trigger >> 32U);
  ...
  if (TimerIdx == HRTIM_TIMERINDEX_TIMER_A)
    { Trigger = ((uint32_t)(pCaptureCfg->Trigger) & 0xFFFF0FFFUL) | ( (TimerF_Trigger ) << HRTIM_CPT1CR_TA1SET_Pos ); }
  else if (TimerIdx == HRTIM_TIMERINDEX_TIMER_B)
    { Trigger = ((uint32_t)(pCaptureCfg->Trigger) & 0xFFF0FFFFUL) | ( (TimerF_Trigger ) << HRTIM_CPT1CR_TB1SET_Pos ); }
  else if (TimerIdx == HRTIM_TIMERINDEX_TIMER_C)
    { Trigger = ((uint32_t)(pCaptureCfg->Trigger) & 0xFF0FFFFFUL) | ( (TimerF_Trigger ) << HRTIM_CPT1CR_TC1SET_Pos ); }
  else if (TimerIdx == HRTIM_TIMERINDEX_TIMER_D)
    { Trigger = ((uint32_t)(pCaptureCfg->Trigger) & 0xF0FFFFFFUL) | ( (TimerF_Trigger ) << HRTIM_CPT1CR_TD1SET_Pos ); }
  else if (TimerIdx == HRTIM_TIMERINDEX_TIMER_E)
    { Trigger = ((uint32_t)(pCaptureCfg->Trigger) & 0x0FFFFFFFUL) | ( (TimerF_Trigger ) << HRTIM_CPT1CR_TE1SET_Pos ); }
  else
    { Trigger = ((uint32_t)(pCaptureCfg->Trigger) & 0xFFFFFFFFUL); }
                                    ...
                                    uint32_t CaptureUnit)
{
  ...
  case HRTIM_CAPTUREUNIT_1:
    {
      captured_value = hhrtim->Instance->sTimerxRegs[TimerIdx].CPT1xR & 0x0000FFFFUL;
      break;
    }

  case HRTIM_CAPTUREUNIT_2:
    {
      captured_value = hhrtim->Instance->sTimerxRegs[TimerIdx].CPT2xR & 0x0000FFFFUL;
      break;
    }
...
/**
  ...
  * @retval None
  */
HAL_HRTIM_WEAK_CALLBACK(void, HAL_HRTIM_Fault1Callback)(HRTIM_HandleTypeDef * const hhrtim)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hhrtim);
...
/**
  ...
  * @retval None
  */
HAL_HRTIM_WEAK_CALLBACK(void, HAL_HRTIM_Fault2Callback)(HRTIM_HandleTypeDef * const hhrtim)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hhrtim);
...
/**
  ...
  * @retval None
  */
HAL_HRTIM_WEAK_CALLBACK(void, HAL_HRTIM_Fault3Callback)(HRTIM_HandleTypeDef * const hhrtim)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hhrtim);
...
/**
  ...
  * @retval None
  */
HAL_HRTIM_WEAK_CALLBACK(void, HAL_HRTIM_Fault4Callback)(HRTIM_HandleTypeDef * const hhrtim)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hhrtim);
...
/**
  ...
  * @retval None
  */
HAL_HRTIM_WEAK_CALLBACK(void, HAL_HRTIM_Fault5Callback)(HRTIM_HandleTypeDef * const hhrtim)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hhrtim);
...
/**
  ...
  * @retval None
  */
__weak void HAL_HRTIM_Fault6Callback(HRTIM_HandleTypeDef * const hhrtim)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hhrtim);
...
/**
  ...
  * @retval None
  */
HAL_HRTIM_WEAK_CALLBACK(void, HAL_HRTIM_SystemFaultCallback)(HRTIM_HandleTypeDef * const hhrtim)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hhrtim);
...
/**
  ...
  * @retval None
  */
__weak void HAL_HRTIM_DLLCalibrationReadyCallback(HRTIM_HandleTypeDef * const hhrtim)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hhrtim);
...
/**
  ...
  * @retval None
  */
HAL_HRTIM_WEAK_CALLBACK(void, HAL_HRTIM_BurstModePeriodCallback)(HRTIM_HandleTypeDef * const hhrtim)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hhrtim);
...
/**
  ...
  * @retval None
  */
HAL_HRTIM_WEAK_CALLBACK(void, HAL_HRTIM_SynchronizationEventCallback)(HRTIM_HandleTypeDef * const hhrtim)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hhrtim);
...
/**
  ...
  * @retval None
  */
HAL_HRTIM_WEAK_CALLBACK(void, HAL_HRTIM_RegistersUpdateCallback)(HRTIM_HandleTypeDef * const hhrtim,
                                              uint32_t TimerIdx)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hhrtim);
...
/**
  ...
  * @retval None
  */
HAL_HRTIM_WEAK_CALLBACK(void, HAL_HRTIM_RepetitionEventCallback)(HRTIM_HandleTypeDef * const hhrtim,
                                              uint32_t TimerIdx)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hhrtim);
...
/**
  ...
  * @retval None
  */
HAL_HRTIM_WEAK_CALLBACK(void, HAL_HRTIM_Compare1EventCallback)(HRTIM_HandleTypeDef * const hhrtim,
                                              uint32_t TimerIdx)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hhrtim);
...
/**
  ...
  *                   @arg HRTIM_TIMERINDEX_TIMER_F for timer F
  */
HAL_HRTIM_WEAK_CALLBACK(void, HAL_HRTIM_Compare2EventCallback)(HRTIM_HandleTypeDef * const hhrtim,
                                              uint32_t TimerIdx)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hhrtim);
...
/**
  ...
  * @retval None
  */
HAL_HRTIM_WEAK_CALLBACK(void, HAL_HRTIM_Compare3EventCallback)(HRTIM_HandleTypeDef * const hhrtim,
                                              uint32_t TimerIdx)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hhrtim);
...
/**
  ...
  * @retval None
  */
HAL_HRTIM_WEAK_CALLBACK(void, HAL_HRTIM_Compare4EventCallback)(HRTIM_HandleTypeDef * const hhrtim,
                                              uint32_t TimerIdx)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hhrtim);
...
/**
  ...
  * @retval None
  */
HAL_HRTIM_WEAK_CALLBACK(void, HAL_HRTIM_Capture1EventCallback)(HRTIM_HandleTypeDef * const hhrtim,
                                              uint32_t TimerIdx)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hhrtim);
...
/**
  ...
  * @retval None
  */
HAL_HRTIM_WEAK_CALLBACK(void, HAL_HRTIM_Capture2EventCallback)(HRTIM_HandleTypeDef * const hhrtim,
                                              uint32_t TimerIdx)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hhrtim);
...
/**
  ...
  * @retval None
  */
HAL_HRTIM_WEAK_CALLBACK(void, HAL_HRTIM_DelayedProtectionCallback)(HRTIM_HandleTypeDef * const hhrtim,
                                              uint32_t TimerIdx)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hhrtim);
...
/**
  ...
  * @retval None
  */
HAL_HRTIM_WEAK_CALLBACK(void, HAL_HRTIM_CounterResetCallback)(HRTIM_HandleTypeDef * const hhrtim,
                                              uint32_t TimerIdx)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hhrtim);
...
/**
  ...
  * @retval None
  */
HAL_HRTIM_WEAK_CALLBACK(void, HAL_HRTIM_Output1SetCallback)(HRTIM_HandleTypeDef * const hhrtim,
                                              uint32_t TimerIdx)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hhrtim);
...
/**
  ...
  * @retval None
  */
HAL_HRTIM_WEAK_CALLBACK(void, HAL_HRTIM_Output1ResetCallback)(HRTIM_HandleTypeDef * const hhrtim,
                                              uint32_t TimerIdx)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hhrtim);
...
/**
  ...
  * @retval None
  */
HAL_HRTIM_WEAK_CALLBACK(void, HAL_HRTIM_Output2SetCallback)(HRTIM_HandleTypeDef * const hhrtim,
                                              uint32_t TimerIdx)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hhrtim);
...
/**
  ...
  * @retval None
  */
HAL_HRTIM_WEAK_CALLBACK(void, HAL_HRTIM_Output2ResetCallback)(HRTIM_HandleTypeDef * const hhrtim,
                                              uint32_t TimerIdx)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hhrtim);
...
/**
  ...
  * @retval None
  */
HAL_HRTIM_WEAK_CALLBACK(void, HAL_HRTIM_BurstDMATransferCallback)(HRTIM_HandleTypeDef * const hhrtim,
                                               uint32_t TimerIdx)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hhrtim);
...
/**
  ...
  * @retval None
  */
HAL_HRTIM_WEAK_CALLBACK(void, HAL_HRTIM_ErrorCallback)(HRTIM_HandleTypeDef * const hhrtim)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hhrtim);
...
static void HRTIM_CaptureUnitConfig(HRTIM_HandleTypeDef * const hhrtim,
                                    ...
                                    uint32_t Event)
{
  uint32_t CaptureTrigger = 0xFFFFFFFFUL;
...
static uint32_t GetTimerIdxFromDMAHandle(HRTIM_HandleTypeDef const * const hhrtim,
                                         DMA_HandleTypeDef const * const hdma)
{
  uint32_t timed_idx = 0xFFFFFFFFUL;
...
static void HRTIM_DMAMasterCplt(DMA_HandleTypeDef * const hdma)
{
  HRTIM_HandleTypeDef * const hrtim = (HRTIM_HandleTypeDef *)((DMA_HandleTypeDef * )hdma)->Parent;
