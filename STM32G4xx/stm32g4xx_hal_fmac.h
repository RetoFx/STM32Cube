...
#if (USE_HAL_FMAC_REGISTER_CALLBACKS == 1U)
  void (* ErrorCallback)(struct __FMAC_HandleTypeDef * const hfmac);               /*!< FMAC error callback                  */

  void (* HalfGetDataCallback)(struct __FMAC_HandleTypeDef * const hfmac);         /*!< FMAC get half data callback          */

  void (* GetDataCallback)(struct __FMAC_HandleTypeDef * const hfmac);             /*!< FMAC get data callback               */

  void (* HalfOutputDataReadyCallback)(struct __FMAC_HandleTypeDef * const hfmac); /*!< FMAC half output data ready callback */

  void (* OutputDataReadyCallback)(struct __FMAC_HandleTypeDef * const hfmac);     /*!< FMAC output data ready callback      */

  void (* FilterConfigCallback)(struct __FMAC_HandleTypeDef * const hfmac);        /*!< FMAC filter configuration callback   */

  void (* FilterPreloadCallback)(struct __FMAC_HandleTypeDef * const hfmac);       /*!< FMAC filter preload callback         */

  void (* MspInitCallback)(struct __FMAC_HandleTypeDef * const hfmac);             /*!< FMAC Msp Init callback               */

  void (* MspDeInitCallback)(struct __FMAC_HandleTypeDef * const hfmac);           /*!< FMAC Msp DeInit callback             */
...
/**
  ...
  */
typedef  void (*pFMAC_CallbackTypeDef)(FMAC_HandleTypeDef * const hfmac);  /*!< pointer to an FMAC callback function */
...
typedef struct
{
  uint8_t                    InputBaseAddress;  /*!< Base address of the input buffer (X1) within the internal memory
                                                     (0x00U to 0xFFU). Ignored if InputBufferSize is set to 0
  ...
  uint8_t                    CoeffBaseAddress;  /*!< Base address of the coefficient buffer (X2) within the internal
                                                     memory (0x00U to 0xFFU). Ignored if CoeffBufferSize is set to 0
                                                     (previous configuration kept).
                                                     Note: the buffers can overlap or even coincide exactly. */
  ...
  uint8_t                    OutputBaseAddress; /*!< Base address of the output buffer (Y) within the internal
                                                     memory (0x00U to 0xFFU). Ignored if OutputBufferSize is set to 0
                                                     (previous configuration kept).
                                                     Note: the buffers can overlap or even coincide exactly. */
...
/** @defgroup FMAC_Data_Buffer_Threshold FMAC Data Buffer Threshold
  ...
  */
#define FMAC_THRESHOLD_1                   0x00000000UL    /*!< Input: Buffer full flag set if the number of free spaces
                                                               in the buffer is less than 1.
                                                               Output: Buffer empty flag set if the number
                                                               of unread values in the buffer is less than 1. */
#define FMAC_THRESHOLD_2                   0x01000000UL    /*!< Input: Buffer full flag set if the number of free spaces
                                                               in the buffer is less than 2.
                                                               Output: Buffer empty flag set if the number
                                                               of unread values in the buffer is less than 2. */
#define FMAC_THRESHOLD_4                   0x02000000UL    /*!< Input: Buffer full flag set if the number of free spaces
                                                               in the buffer is less than 4.
                                                               Output: Buffer empty flag set if the number
                                                               of unread values in the buffer is less than 4. */
#define FMAC_THRESHOLD_8                   0x03000000UL    /*!< Input: Buffer full flag set if the number of free spaces
...
void HAL_FMAC_MspInit(FMAC_HandleTypeDef * const hfmac);
void HAL_FMAC_MspDeInit(FMAC_HandleTypeDef * const hfmac);
...
/** @addtogroup FMAC_Exported_Functions_Group3
  ...
  */
/* Callback functions *********************************************************/
void HAL_FMAC_ErrorCallback(FMAC_HandleTypeDef * const hfmac);
void HAL_FMAC_HalfGetDataCallback(FMAC_HandleTypeDef * const hfmac);
void HAL_FMAC_GetDataCallback(FMAC_HandleTypeDef const * const hfmac);
void HAL_FMAC_HalfOutputDataReadyCallback(FMAC_HandleTypeDef * const hfmac);
void HAL_FMAC_OutputDataReadyCallback(FMAC_HandleTypeDef * const hfmac);
void HAL_FMAC_FilterConfigCallback(FMAC_HandleTypeDef * const hfmac);
void HAL_FMAC_FilterPreloadCallback(FMAC_HandleTypeDef * const hfmac);
/**
  * @}
  */
