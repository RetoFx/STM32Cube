...
#define SCnSCB              __PERIPHERIE(SCnSCB_Type,    SCS_BASE      )   /*!< System control Register not in SCB */
#define SCB                 __PERIPHERIE(SCB_Type,       SCB_BASE      )   /*!< SCB configuration struct */
#define SysTick             __PERIPHERIE(SysTick_Type,   SysTick_BASE  )   /*!< SysTick configuration struct */
#define NVIC                __PERIPHERIE(NVIC_Type,      NVIC_BASE     )   /*!< NVIC configuration struct */
#define ITM                 __PERIPHERIE(ITM_Type,       ITM_BASE      )   /*!< ITM configuration struct */
#define DWT                 __PERIPHERIE(DWT_Type,       DWT_BASE      )   /*!< DWT configuration struct */
#define TPIU                __PERIPHERIE(TPIU_Type,      TPIU_BASE     )   /*!< TPIU configuration struct */
#define DCB                 __PERIPHERIE(DCB_Type,       DCB_BASE      )   /*!< DCB configuration struct */

#if defined (__MPU_PRESENT) && (__MPU_PRESENT == 1U)
  #define MPU_BASE          (SCS_BASE +  0x0D90UL)                    /*!< Memory Protection Unit */
  #define MPU               __PERIPHERIE(MPU_Type,       MPU_BASE      )   /*!< Memory Protection Unit */
#endif

#define FPU_BASE            (SCS_BASE +  0x0F30UL)                    /*!< Floating Point Unit */
#define FPU                 __PERIPHERIE(FPU_Type,       FPU_BASE      )   /*!< Floating Point Unit */
...
#define CoreDebug           __PERIPHERIE(CoreDebug_Type, DCB_BASE)

#endif // CMSIS_DISABLE_DEPRECATED

#if defined(__PERIPHERIE_STRUCT)
 #if !defined(CORE_PERIPHERIE_H)
  #define CORE_PERIPHERIE_H	"../Windows/core_peripherie.h"
 #endif
 #include CORE_PERIPHERIE_H
#endif
...
{
  uint32_t reg_value;
  uint32_t const PriorityGroupTmp = (PriorityGroup & (uint32_t)0x07UL);       /* only values 0..7 are used          */
...
{
  uint32_t * const vectors = (uint32_t *) ((uintptr_t) SCB->VTOR);
...
{
  uint32_t const * const vectors = (uint32_t const *) ((uintptr_t) SCB->VTOR);
...
#define                 ITM_RXBUFFER_EMPTY  ((int32_t)0x5AA55AA5UL) /*!< Value identifying \ref ITM_RxBuffer is ready for next character. */
