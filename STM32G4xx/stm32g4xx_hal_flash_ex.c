...
static void              FLASH_OB_GetSecMem(uint32_t SecMemBank, uint32_t * const SecMemSize);
...
/**
  ...
  * @retval None
  */
static void FLASH_OB_GetSecMem(uint32_t SecBank, uint32_t * const SecSize)
{
...
static void FLASH_OB_GetPCROP(uint32_t * const PCROPConfig, uint32_t * const PCROPStartAddr, uint32_t * const PCROPEndAddr)
{
  ...
  if (READ_BIT(FLASH->OPTR, FLASH_OPTR_DBANK) == 0U)
  {
    if (((*PCROPConfig) & FLASH_BANK_BOTH) == FLASH_BANK_1)
    {
      ...
      *PCROPEndAddr = (reg_value << 4U) + FLASH_BASE;
    }
    else if (((*PCROPConfig) & FLASH_BANK_BOTH) == FLASH_BANK_2)
    {
...
#endif
  {
    if (((*PCROPConfig) & FLASH_BANK_BOTH) == FLASH_BANK_1)
    {
      ...
      *PCROPStartAddr = (reg_value << 3U) + bank1_addr;
      ...
      reg_value     = (READ_REG(FLASH->PCROP1ER) & FLASH_PCROP1ER_PCROP1_END);
      *PCROPEndAddr = (reg_value << 3U) + bank1_addr;
    }
#if defined (FLASH_OPTR_DBANK)
    else if (((*PCROPConfig) & FLASH_BANK_BOTH) == FLASH_BANK_2)
    {
      ...
      *PCROPStartAddr = (reg_value << 3U) + bank2_addr;
      ...
      reg_value     = (READ_REG(FLASH->PCROP2ER) & FLASH_PCROP2ER_PCROP2_END);
      *PCROPEndAddr = (reg_value << 3U) + bank2_addr;
    }
#endif
    else
    {
