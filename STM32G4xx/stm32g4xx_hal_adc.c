...
HAL_StatusTypeDef HAL_ADC_Init(ADC_HandleTypeDef * const hadc)
{
     ...
     )
  {
    ...
    /*   0x00U: 1 channel converted (channel on regular rank 1)                */
    ...
    if (hadc->Init.ScanConvMode == ADC_SCAN_ENABLE)
    {
      ...
      MODIFY_REG(hadc->Instance->SQR1, ADC_SQR1_L, (hadc->Init.NbrOfConversion - (uint8_t)1U));
    }
    else
    {
...
HAL_StatusTypeDef HAL_ADC_DeInit(ADC_HandleTypeDef * const hadc)
{
  ...
  if (__LL_ADC_IS_ENABLED_ALL_COMMON_INSTANCE(__LL_ADC_COMMON_INSTANCE(hadc->Instance)) == 0UL)
  {
    ...
    */
    ...
    /* in function "void HAL_ADC_MspDeInit(ADC_HandleTypeDef * const hadc)":         */
#if (USE_HAL_ADC_REGISTER_CALLBACKS == 1U)
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_ADC_AnalogWDGConfig(ADC_HandleTypeDef * const hadc, ADC_AnalogWDGConfTypeDef const * const pAnalogWDGConfig)
{
  HAL_StatusTypeDef tmp_hal_status = HAL_OK;
