...
static void HCD_RXQLVL_IRQHandler(HCD_HandleTypeDef * const hhcd);
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_HCD_HC_Init(HCD_HandleTypeDef * const hhcd, uint8_t ch_num, uint8_t epnum,
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_HCD_HC_SubmitRequest(HCD_HandleTypeDef * const hhcd,
                                           uint8_t ch_num,
                                           uint8_t direction,
                                           uint8_t ep_type,
                                           uint8_t token,
...
/**
  ...
  * @retval None
  */
HAL_HCD_WEAK_CALLBACK(void, HAL_HCD_HC_NotifyURBChange_Callback)(HCD_HandleTypeDef * const hhcd, uint8_t chnum, HCD_URBStateTypeDef urb_state)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hhcd);
  UNUSED(chnum);
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_HCD_UnRegisterCallback(HCD_HandleTypeDef * const hhcd, HAL_HCD_CallbackIDTypeDef CallbackID)
{
  HAL_StatusTypeDef status = HAL_OK;
...
static void HCD_HC_IN_IRQHandler(HCD_HandleTypeDef * const hhcd, uint8_t chnum)
{
  USB_OTG_GlobalTypeDef const * const USBx = hhcd->Instance;
  uint32_t USBx_BASE = (uint32_t)USBx;
  uint32_t tmpreg;
...
static void HCD_RXQLVL_IRQHandler(HCD_HandleTypeDef * const hhcd)
{
  ...
  pktsts = (GrxstspReg & USB_OTG_GRXSTSP_PKTSTS) >> 17U;
  pktcnt = (GrxstspReg & USB_OTG_GRXSTSP_BCNT) >> 4U;
    ...
    case GRXSTS_PKTSTS_IN:
      ...
      if ((pktcnt > 0U) && (hhcd->hc[chnum].xfer_buff != (void *)0))
      {
        if ((hhcd->hc[chnum].xfer_count + pktcnt) <= hhcd->hc[chnum].xfer_len)
        {
          ...
          xferSizePktCnt = (USBx_HC(chnum)->HCTSIZ & USB_OTG_HCTSIZ_PKTCNT) >> 19U;
