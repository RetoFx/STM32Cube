...
/** @defgroup ETH_Private_Functions ETH Private Functions
  * @{
  */
static void ETH_MACDMAConfig(ETH_HandleTypeDef * const heth, uint32_t err);
static void ETH_MACAddressConfig(ETH_HandleTypeDef * const heth, uint32_t MacAddr, uint8_t const * const Addr);
static void ETH_MACReceptionEnable(ETH_HandleTypeDef * const heth);
static void ETH_MACReceptionDisable(ETH_HandleTypeDef * const heth);
static void ETH_MACTransmissionEnable(ETH_HandleTypeDef * const heth);
static void ETH_MACTransmissionDisable(ETH_HandleTypeDef * const heth);
static void ETH_DMATransmissionEnable(ETH_HandleTypeDef * const heth);
static void ETH_DMATransmissionDisable(ETH_HandleTypeDef * const heth);
static void ETH_DMAReceptionEnable(ETH_HandleTypeDef * const heth);
static void ETH_DMAReceptionDisable(ETH_HandleTypeDef * const heth);
...
HAL_StatusTypeDef HAL_ETH_Init(ETH_HandleTypeDef * const heth)
{
  ...
  else /* ((hclk >= 100000000UL) && (hclk < 120000000UL)) */
  {
...
static void ETH_Delay(uint32_t mdelay)
{
  __IO uint32_t Delay = mdelay * (SystemCoreClock / 8UL / 1000UL);
  do
  {
    __NOP();
  }
