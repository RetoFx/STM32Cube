...
#define IS_LL_DMA_NBDATA(__VALUE__)             ((__VALUE__)  <= (uint32_t)0x0000FFFFUL)
...
uint32_t LL_DMA_DeInit(DMA_TypeDef * const DMAx, uint32_t Stream)
{
  ...
  else
  {
    ...
    LL_DMA_WriteReg(tmp, FCR, 0x00000021UL);
...
void LL_DMA_StructInit(LL_DMA_InitTypeDef * const DMA_InitStruct)
{
  ...
  DMA_InitStruct->PeriphOrM2MSrcAddress  = 0x00000000UL;
  DMA_InitStruct->MemoryOrM2MDstAddress  = 0x00000000UL;
  ...
  DMA_InitStruct->NbData                 = 0x00000000UL;
