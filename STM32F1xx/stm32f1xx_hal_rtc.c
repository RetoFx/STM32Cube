...
/** @defgroup RTC_Private_Constants RTC Private Constants
  * @{
  */
#define RTC_ALARM_RESETVALUE_REGISTER    ((uint16_t)0xFFFFU)
#define RTC_ALARM_RESETVALUE             0xFFFFFFFFUL
...
/** @defgroup RTC_Private_Functions RTC Private Functions
  * @{
  */
static uint32_t           RTC_ReadTimeCounter(RTC_HandleTypeDef const * const hrtc);
static HAL_StatusTypeDef  RTC_WriteTimeCounter(RTC_HandleTypeDef const * const hrtc, uint32_t TimeCounter);
static uint32_t           RTC_ReadAlarmCounter(RTC_HandleTypeDef const * const hrtc);
static HAL_StatusTypeDef  RTC_WriteAlarmCounter(RTC_HandleTypeDef const * const hrtc, uint32_t AlarmCounter);
static HAL_StatusTypeDef  RTC_EnterInitMode(RTC_HandleTypeDef const * const hrtc);
static HAL_StatusTypeDef  RTC_ExitInitMode(RTC_HandleTypeDef const * const hrtc);
static uint8_t            RTC_ByteToBcd2(uint8_t Value);
static uint8_t            RTC_Bcd2ToByte(uint8_t Value);
static uint8_t            RTC_IsLeapYear(uint16_t nYear);
static void               RTC_DateUpdate(RTC_HandleTypeDef * const hrtc, uint32_t DayElapsed);
...
HAL_StatusTypeDef HAL_RTC_DeInit(RTC_HandleTypeDef * const hrtc)
{
  ...
  else
  {
    ...
    WRITE_REG(hrtc->Instance->PRLL, 0x00008000UL);
