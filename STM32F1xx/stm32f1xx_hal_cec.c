...
#define CEC_RXXFERSIZE_INITIALIZE 0xFFFFU /*!< Value used to initialise the RxXferSize of the handle */
/**
  * @}
  */
...
/** @defgroup CEC_Private_Functions CEC Private Functions
  * @{
  */
static HAL_StatusTypeDef CEC_Transmit_IT(CEC_HandleTypeDef * const hcec);
static HAL_StatusTypeDef CEC_Receive_IT(CEC_HandleTypeDef * const hcec);
/**
  * @}
  */
...
HAL_StatusTypeDef HAL_CEC_Init(CEC_HandleTypeDef * const hcec)
{
  ...
  MODIFY_REG(hcec->Instance->PRES, CEC_PRES_PRES, 50U * (HAL_RCC_GetPCLK1Freq() / 1000000UL) - 1U);
...
/**
  ...
  * @retval None
  */
__weak void HAL_CEC_MspInit(CEC_HandleTypeDef * const hcec)
{
...
/**
  ...
  * @retval None
  */
__weak void HAL_CEC_MspDeInit(CEC_HandleTypeDef * const hcec)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hcec);
  /* NOTE : This function should not be modified, when the callback is needed,
...
@verbatim
 ...
 ===============================================================================
    ...
    (#) The CEC handle must contain the initiator (TX side) and the destination (RX side)
        logical addresses (4-bit long addresses, 0xFU for broadcast messages destination)
...
/**
  ...
  * @retval None
  */
HAL_CEC_WEAK_CALLBACK(void, HAL_CEC_TxCpltCallback)(CEC_HandleTypeDef * const hcec)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hcec);
  /* NOTE : This function should not be modified, when the callback is needed,
...
/**
  ...
  * @retval None
  */
HAL_CEC_WEAK_CALLBACK(void, HAL_CEC_RxCpltCallback)(CEC_HandleTypeDef * const hcec, uint32_t RxFrameSize)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hcec);
  UNUSED(RxFrameSize);
...
/**
  ...
  * @retval None
  */
HAL_CEC_WEAK_CALLBACK(void, HAL_CEC_ErrorCallback)(CEC_HandleTypeDef * const hcec)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hcec);
  /* NOTE : This function should not be modified, when the callback is needed,
...
static HAL_StatusTypeDef CEC_Transmit_IT(CEC_HandleTypeDef * const hcec)
{
  ...
  if ((hcec->gState == HAL_CEC_STATE_BUSY_TX) || (__HAL_CEC_GET_TRANSMISSION_START_FLAG(hcec) != RESET))
  {
    ...
    {
      /* Acknowledge successful completion by writing 0x00U */
    ...
    else
    {
      ...
      {
        /* Acknowledge byte request by writing 0x00U */
        MODIFY_REG(hcec->Instance->CSR, CEC_FLAG_TRANSMIT_MASK, 0x00U);
      }
...
static HAL_StatusTypeDef CEC_Receive_IT(CEC_HandleTypeDef * const hcec)
{
  ...
  if (hcec->RxState == HAL_CEC_STATE_BUSY_RX)
  {
    ...
    *hcec->Init.RxBuffer++ = hcec->Instance->RXD;
    ...
    /* Acknowledge received byte by writing 0x00U */
