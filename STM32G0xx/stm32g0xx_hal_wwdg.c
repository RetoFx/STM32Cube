/**
  ...
  ******************************************************************************
  ...
  [..]
    ...
    before reaching 0x3FU value (i.e. a reset is generated when the counter
    value rolls down from 0x40U to 0x3FU).
    ...
    (+) If required by application, an Early Wakeup Interrupt can be triggered
        ...
        reaches 0x40U, interrupt occurs. This mechanism requires WWDG interrupt
    ...
    (+) WWDG Counter refresh is allowed between the following limits :
        ...
        (++) max time (mS) = 1000 * (Counter - 0x40U) / WWDG clock
    (+) Typical values:
        (++) Counter min (T[5;0] = 0x00U) at 64 MHz (PCLK1) with zero prescaler:
             max timeout before reset: approximately 64us
        (++) Counter max (T[5;0] = 0x3FU) at 64 MHz (PCLK1) with prescaler
             dividing by 128:
  ...
  [..]
    ...
    (+) Configure the WWDG prescaler, refresh window value, counter value and early
        ...
        greater than 0x40U to prevent generation of immediate reset.
    (+) If the Early Wakeup Interrupt (EWI) feature is enabled, an interrupt is
        generated when the counter reaches 0x40U. When HAL_WWDG_IRQHandler is
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_WWDG_Init(WWDG_HandleTypeDef * const hwwdg)
{
...
#if (USE_HAL_WWDG_REGISTER_CALLBACKS == 1U)
...
/**
  ...
  * @retval None
  */
__weak void HAL_WWDG_MspInit(WWDG_HandleTypeDef * const hwwdg)
{
...
/**
  ...
  * @retval status
  */
HAL_StatusTypeDef HAL_WWDG_RegisterCallback(WWDG_HandleTypeDef * const hwwdg, HAL_WWDG_CallbackIDTypeDef CallbackID,
...
/**
  ...
  * @retval status
  */
HAL_StatusTypeDef HAL_WWDG_UnRegisterCallback(WWDG_HandleTypeDef * const hwwdg, HAL_WWDG_CallbackIDTypeDef CallbackID)
{
  HAL_StatusTypeDef status = HAL_OK;
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_WWDG_Refresh(WWDG_HandleTypeDef * const hwwdg)
{
...
/**
  ...
  *         When the downcounter reaches the value 0x40U, and EWI interrupt is
  ...
  * @retval None
  */
void HAL_WWDG_IRQHandler(WWDG_HandleTypeDef * const hwwdg)
{
...
/**
  ...
  * @retval None
  */
HAL_WWDG_WEAK_CALLBACK(void, HAL_WWDG_EarlyWakeupCallback)(WWDG_HandleTypeDef * const hwwdg)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hwwdg);
