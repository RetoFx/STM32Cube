...
/*                - LPUART_BRR register value should be >= 0x300U              */
/*                - LPUART_BRR register value should be <= 0xFFFFFUL (20 bits)  */
/*              Baudrate specified by the user should belong to [8, 21300000].*/
#define IS_LL_LPUART_BAUDRATE(__BAUDRATE__) (((__BAUDRATE__) <= 21300000UL) && ((__BAUDRATE__) >= 8U))

/* __VALUE__ BRR content must be greater than or equal to 0x300U. */
#define IS_LL_LPUART_BRR_MIN(__VALUE__)   ((__VALUE__) >= 0x300U)

/* __VALUE__ BRR content must be lower than or equal to 0xFFFFFUL. */
#define IS_LL_LPUART_BRR_MAX(__VALUE__)   ((__VALUE__) <= 0x000FFFFFUL)
  ...
  if (LL_LPUART_IsEnabled(LPUARTx) == 0U)
  {
    ...
    */
        ...
        && (LPUART_InitStruct->BaudRate != 0U))
    {
      ...
      /* Check BRR is greater than or equal to 0x300U */
      assert_param(IS_LL_LPUART_BRR_MIN(LPUARTx->BRR));

      /* Check BRR is lower than or equal to 0xFFFFFUL */
