...
/** @defgroup HCD_Private_Functions HCD Private Functions
  * @{
  */
static void HCD_HC_IN_IRQHandler(HCD_HandleTypeDef * const hhcd, uint8_t chnum);
static void HCD_HC_OUT_IRQHandler(HCD_HandleTypeDef * const hhcd, uint8_t chnum);
static void HCD_Port_IRQHandler(HCD_HandleTypeDef * const hhcd);
static void HAL_HCD_ClearPhyChannel(HCD_HandleTypeDef * const hhcd);
static uint8_t HAL_HCD_GetLogical_Channel(HCD_HandleTypeDef const * const hhcd, uint8_t phy_chnum, uint8_t dir);
static uint8_t HAL_HCD_Check_usedChannel(HCD_HandleTypeDef const * const hhcd, uint8_t ch_num);
static uint8_t HAL_HCD_Get_FreePhyChannel(HCD_HandleTypeDef * const hhcd, uint8_t ch_num, uint8_t epnum, uint8_t ep_type);

#if (USE_USB_DOUBLE_BUFFER == 1U)
static void HCD_HC_IN_BulkDb(HCD_HandleTypeDef * const hhcd, uint8_t ch_num, uint8_t phy_chnum, uint32_t regvalue);
static void HCD_HC_OUT_BulkDb(HCD_HandleTypeDef * const hhcd, uint8_t ch_num, uint8_t phy_chnum, uint32_t regvalue);
#endif /* (USE_USB_DOUBLE_BUFFER == 1U) */

static uint16_t HAL_HCD_GetFreePMA(HCD_HandleTypeDef * const hhcd, uint16_t mps);
static HAL_StatusTypeDef  HAL_HCD_PMAFree(HCD_HandleTypeDef * const hhcd, uint32_t pma_base, uint16_t mps);
static void inline HCD_HC_IN_ISO(HCD_HandleTypeDef * const hhcd, uint8_t ch_num, uint8_t phy_chnum, uint32_t regvalue);
/**
  * @}
  */
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_HCD_Init(HCD_HandleTypeDef * const hhcd)
{
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_HCD_HC_Init(HCD_HandleTypeDef * const hhcd, uint8_t ch_num,
    ...
    else
    {
      ...
      else
      {
        if (ch_num == 0U)
        {
          ...
          if ((ep0_virtual_channel != 0U) && (((hhcd->ep0_PmaAllocState & 0xF0U) >> 4U) == CH_IN_DIR))
          {
          ...
          else
          {
            ...
            hhcd->ep0_PmaAllocState |= (0x1UL << 12U);
        ...
        {
          if (((hhcd->ep0_PmaAllocState & 0xF00U) >> 8U) == 1U)
          {
            ep0_virtual_channel = (uint8_t)(hhcd->ep0_PmaAllocState & 0xFU);

            if (((hhcd->ep0_PmaAllocState & 0xF0U) >> 4U) == CH_IN_DIR)
            {
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_HCD_HC_Close(HCD_HandleTypeDef * const hhcd, uint8_t ch_num)
{
  /* Stop the channel */
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_HCD_HC_Halt(HCD_HandleTypeDef * const hhcd, uint8_t ch_num)
{
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_HCD_DeInit(HCD_HandleTypeDef * const hhcd)
{
  uint8_t idx;
...
/**
  ...
  * @retval None
  */
__weak void  HAL_HCD_MspInit(HCD_HandleTypeDef * const hhcd)
{
...
/**
  ...
  * @retval None
  */
__weak void  HAL_HCD_MspDeInit(HCD_HandleTypeDef * const hhcd)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hhcd);
...
__weak void HAL_HCD_SuspendCallback(HCD_HandleTypeDef * const hhcd)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hhcd);
...
__weak void HAL_HCD_ResumeCallback(HCD_HandleTypeDef * const hhcd)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hhcd);
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_HCD_HC_SubmitRequest(HCD_HandleTypeDef * const hhcd, uint8_t ch_num,
                                           uint8_t direction, uint8_t ep_type,
                                           uint8_t token, uint8_t * const pbuff,
...
/**
  ...
  * @retval None
  */
void HAL_HCD_IRQHandler(HCD_HandleTypeDef * const hhcd)
{
  uint8_t phy_chnum;
  uint8_t ch_dir;
  ...
  if ((SYSCFG->IT_LINE_SR[8U] & (0x1UL << 2U)) == 0U)
  {
    return;
  }
...
/**
  ...
  * @retval None
  */
HAL_HCD_WEAK_CALLBACK(void, HAL_HCD_SOF_Callback)(HCD_HandleTypeDef * const hhcd)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hhcd);
...
/**
  ...
  * @retval None
  */
HAL_HCD_WEAK_CALLBACK(void, HAL_HCD_Connect_Callback)(HCD_HandleTypeDef * const hhcd)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hhcd);
...
/**
  ...
  * @retval None
  */
HAL_HCD_WEAK_CALLBACK(void, HAL_HCD_Disconnect_Callback)(HCD_HandleTypeDef * const hhcd)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hhcd);
...
/**
  ...
  * @retval None
  */
__weak void HAL_HCD_PortEnabled_Callback(HCD_HandleTypeDef * const hhcd)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hhcd);
...
/**
  ...
  * @retval None
  */
__weak void HAL_HCD_PortDisabled_Callback(HCD_HandleTypeDef * const hhcd)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hhcd);
...
/**
  ...
  * @retval None
  */
HAL_HCD_WEAK_CALLBACK(void, HAL_HCD_HC_NotifyURBChange_Callback)(HCD_HandleTypeDef * const hhcd,
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_HCD_RegisterCallback(HCD_HandleTypeDef * const hhcd,
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_HCD_UnRegisterCallback(HCD_HandleTypeDef * const hhcd,
                                             HAL_HCD_CallbackIDTypeDef CallbackID)
{
  HAL_StatusTypeDef status = HAL_OK;
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_HCD_RegisterHC_NotifyURBChangeCallback(HCD_HandleTypeDef * const hhcd,
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_HCD_UnRegisterHC_NotifyURBChangeCallback(HCD_HandleTypeDef * const hhcd)
{
  HAL_StatusTypeDef status = HAL_OK;
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_HCD_Start(HCD_HandleTypeDef * const hhcd)
{
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_HCD_Stop(HCD_HandleTypeDef * const hhcd)
{
  __HAL_LOCK(hhcd);
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_HCD_Suspend(HCD_HandleTypeDef * const hhcd)
{
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_HCD_Resume(HCD_HandleTypeDef * const hhcd)
{
  /* Set Resume bit */
  hhcd->Instance->CNTR |= USB_CNTR_L2RES;
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_HCD_ResetPort(HCD_HandleTypeDef * const hhcd)
{
  __HAL_LOCK(hhcd);
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_HCD_ResumePort(HCD_HandleTypeDef * const hhcd)
{
  /* Set Resume bit */
  hhcd->Instance->CNTR |= USB_CNTR_L2RES;
  HAL_Delay(30U);
...
/**
  ...
  * @retval HAL state
  */
HCD_StateTypeDef HAL_HCD_GetState(HCD_HandleTypeDef const * const hhcd)
{
  return hhcd->State;
}
...
/**
  ...
  *            URB_STALL
  */
HCD_URBStateTypeDef HAL_HCD_HC_GetURBState(HCD_HandleTypeDef const * const hhcd, uint8_t chnum)
{
...
/**
  ...
  */
uint32_t HAL_HCD_HC_GetXferCount(HCD_HandleTypeDef const * const hhcd, uint8_t chnum)
{
...
/**
  ...
  *            HC_DATATGLERR
  */
HCD_HCStateTypeDef  HAL_HCD_HC_GetState(HCD_HandleTypeDef const * const hhcd, uint8_t chnum)
{
...
/**
  ...
  */
uint32_t HAL_HCD_GetCurrentFrame(HCD_HandleTypeDef const * const hhcd)
{
...
/**
  ...
  */
uint32_t HAL_HCD_GetCurrentSpeed(HCD_HandleTypeDef const * const hhcd)
{
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_HCD_HC_SetHubInfo(HCD_HandleTypeDef * const hhcd, uint8_t ch_num,
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_HCD_HC_ClearHubInfo(HCD_HandleTypeDef * const hhcd, uint8_t ch_num)
{
...
/**
  ...
  * @retval none
  */
static void HCD_HC_OUT_BulkDb(HCD_HandleTypeDef * const hhcd, uint8_t ch_num,
                              uint8_t phy_chnum, uint32_t regvalue)
{
  ...
  {
    data_xfr = (uint16_t)(((USB_DRD_PMA_BUFF + phy_chnum)->TXBD & 0x03FF0000UL) >> 16U);
  ...
  {
    /* Send Buffer1 */
    data_xfr = (uint16_t)(((USB_DRD_PMA_BUFF + phy_chnum)->RXBD & 0x03FF0000UL) >> 16U);
...
/**
  ...
  * @retval none
  */
static void HCD_HC_IN_BulkDb(HCD_HandleTypeDef * const hhcd,
...
/**
  ...
  * @retval none
  */
static void inline HCD_HC_IN_ISO(HCD_HandleTypeDef * const hhcd, uint8_t ch_num,
                                 uint8_t phy_chnum, uint32_t regvalue)
{
...
/**
  ...
  * @retval HAL status
  */
static uint8_t HAL_HCD_GetLogical_Channel(HCD_HandleTypeDef const * const hhcd,
...
/**
  ...
  */
static uint8_t HAL_HCD_Get_FreePhyChannel(HCD_HandleTypeDef * const hhcd, uint8_t ch_num,
...
/**
  ...
  * @retval PMA_Address of the first free block containing mps byte
            0xFFFFU in case of no space available
  */
static uint16_t HAL_HCD_GetFreePMA(HCD_HandleTypeDef * const hhcd, uint16_t mps)
{
  uint32_t Entry;
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef  HAL_HCD_PMAlloc(HCD_HandleTypeDef * const hhcd, uint8_t ch_num,
                                   uint16_t ch_kind, uint16_t mps)
{
  ...
  /* Host Channel */
  HCD_HCTypeDef * const hc = &(hhcd->hc[ch_num]);
  ...
  else
  {
    ...
    if (ch_kind == HCD_SNG_BUF)
    {
      ...
      if (hc->ch_dir == CH_IN_DIR)
      {
        ...
        if (hc->ep_num == 0U)
        {
          hhcd->ep0_PmaAllocState |= (CH_IN_DIR << 4U);
        }
      }
      else
      {
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef  HAL_HCD_PMADeAlloc(HCD_HandleTypeDef * const hhcd, uint8_t ch_num)
{
  HAL_StatusTypeDef status;
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef  HAL_HCD_PMAReset(HCD_HandleTypeDef * const hhcd)
{
  /* Reset All PMA Entry */
  ...
  {
    hhcd->PMALookupTable[0U] |= ((uint32_t)1UL << i);
  }
