...
#define CRS_CFGR_FELIM_BIT_NUMBER    16U
#define CRS_CR_TRIM_BIT_NUMBER       8U
#define CRS_ISR_FECAP_BIT_NUMBER     16U
/**
  * @}
  */
#endif /* CRS */
...
HAL_StatusTypeDef HAL_RCCEx_PeriphCLKConfig(RCC_PeriphCLKInitTypeDef const * const PeriphClkInit)
{
  ...
  if(((PeriphClkInit->PeriphClockSelection) & RCC_PERIPHCLK_RTC) == (RCC_PERIPHCLK_RTC))
  {
    ...
    if((temp_reg != 0x00000000UL) && (temp_reg != (PeriphClkInit->RTCClockSelection & RCC_BDCR_RTCSEL)))
    {
  ...
  case RCC_PERIPHCLK_USB:
    {
      ...
      if ((srcclk == RCC_USBCLKSOURCE_PLL) && (HAL_IS_BIT_SET(RCC->CR, RCC_CR_PLLRDY)))
      {
        ...
        pllmull      = (pllmull >> RCC_CFGR_PLLMUL_BIT_NUMBER) + 2U;
...
void HAL_RCCEx_CRSConfig(RCC_CRSInitTypeDef const * const pInit)
{
  ...
  value |= (pInit->ErrorLimitValue << CRS_CFGR_FELIM_BIT_NUMBER);
  WRITE_REG(CRS->CFGR, value);

  /* Adjust HSI48 oscillator smooth trimming */
  /* Set the TRIM[5:0] bits according to RCC_CRS_HSI48CalibrationValue value */
  MODIFY_REG(CRS->CR, CRS_CR_TRIM, (pInit->HSI48CalibrationValue << CRS_CR_TRIM_BIT_NUMBER));
...
void HAL_RCCEx_CRSGetSynchronizationInfo(RCC_CRSSynchroInfoTypeDef * const pSynchroInfo)
{
  ...
  pSynchroInfo->HSI48CalibrationValue = (uint32_t)(READ_BIT(CRS->CR, CRS_CR_TRIM) >> CRS_CR_TRIM_BIT_NUMBER);

  /* Get Frequency error capture */
  pSynchroInfo->FreqErrorCapture = (uint32_t)(READ_BIT(CRS->ISR, CRS_ISR_FECAP) >> CRS_ISR_FECAP_BIT_NUMBER);
...
/**
  ...
  * @retval none
  */
HAL_RCCEX_WEAK_CALLBACK(void, HAL_RCCEx_CRS_SyncOkCallback)(void)
{
...
/**
  ...
  * @retval none
  */
HAL_RCCEX_WEAK_CALLBACK(void, HAL_RCCEx_CRS_SyncWarnCallback)(void)
{
  /* NOTE : This function should not be modified, when the callback is needed,
...
/**
  ...
  * @retval none
  */
HAL_RCCEX_WEAK_CALLBACK(void, HAL_RCCEx_CRS_ExpectedSyncCallback)(void)
{
  /* NOTE : This function should not be modified, when the callback is needed,
...
/**
  ...
  * @retval none
  */
HAL_RCCEX_WEAK_CALLBACK(void, HAL_RCCEx_CRS_ErrorCallback)(uint32_t Error)
{
