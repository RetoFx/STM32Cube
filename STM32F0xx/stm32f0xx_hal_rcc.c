...
uint32_t HAL_RCC_GetSysClockFreq(void)
{
  ...
  switch (tmpreg & RCC_CFGR_SWS)
  {
    ...
    {
      pllmul = aPLLMULFactorTable[(uint32_t)(tmpreg & RCC_CFGR_PLLMUL) >> RCC_CFGR_PLLMUL_BIT_NUMBER];
      prediv = aPredivFactorTable[(uint32_t)(RCC->CFGR2 & RCC_CFGR2_PREDIV) >> RCC_CFGR2_PREDIV_BIT_NUMBER];
...
uint32_t HAL_RCC_GetPCLK1Freq(void)
{
  ...
  return (HAL_RCC_GetHCLKFreq() >> APBPrescTable[(RCC->CFGR & RCC_CFGR_PPRE) >> RCC_CFGR_PPRE_BIT_NUMBER]);
}
...
void HAL_RCC_GetOscConfig(RCC_OscInitTypeDef * const RCC_OscInitStruct)
{
  ...
  RCC_OscInitStruct->HSICalibrationValue = (uint32_t)((RCC->CR &RCC_CR_HSITRIM) >> RCC_CR_HSITRIM_BIT_NUMBER);
