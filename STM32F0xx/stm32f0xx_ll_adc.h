...
#define VREFINT_CAL_ADDR                   ((uint16_t *) (0x1FFFF7BAUL)) /* Internal voltage reference, address of parameter VREFINT_CAL: VrefInt ADC raw data acquired at temperature 30 DegC (tolerance: +-5 DegC), Vref+ = 3.3 V (tolerance: +-10 mV). */
...
/* Temperature sensor */
#define TEMPSENSOR_CAL1_ADDR               ((uint16_t *) (0x1FFFF7B8UL)) /* Internal temperature sensor, address of parameter TS_CAL1: On STM32F0, temperature sensor ADC raw data acquired at temperature  30 DegC (tolerance: +-5 DegC), Vref+ = 3.3 V (tolerance: +-10 mV). */
#define TEMPSENSOR_CAL2_ADDR               ((uint16_t *) (0x1FFFF7C2UL)) /* Internal temperature sensor, address of parameter TS_CAL2: On STM32F0, temperature sensor ADC raw data acquired at temperature 110 DegC (tolerance: +-5 DegC), Vref+ = 3.3 V (tolerance: +-10 mV). */
...
#define LL_ADC_CLOCK_ASYNC                 (0x00000000UL)                               /*!< ADC asynchronous clock. On this STM32 series, asynchronous clock has no prescaler. */
/**
  * @}
  */
...
__STATIC_INLINE void LL_ADC_SetAnalogWDThresholds(ADC_TypeDef * const ADCx, uint32_t AWDThresholdsHighLow, uint32_t AWDThresholdValue)
{
  ...
  MODIFY_REG(ADCx->TR,
             ...
             AWDThresholdValue << ((AWDThresholdsHighLow >> ADC_TR_HT_BITOFFSET_POS) & 0x00000010UL));
}
