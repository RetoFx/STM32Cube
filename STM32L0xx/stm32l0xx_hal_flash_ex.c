...
static HAL_StatusTypeDef FLASH_OB_RDPConfig(uint8_t OB_RDP)
{
  ...
  tmp1 = (uint32_t)(OB->RDP & ((~FLASH_OPTR_RDPROT) & 0x0000FFFFUL));
...
{
  HAL_StatusTypeDef status = HAL_OK;
  uint32_t tmp = 0, tmp1 = 0, OB_Bits = ((uint32_t) OB_BootBit1) << 15U;
...
#endif
{
  ...
  if(status == HAL_OK)
  {
    ...
    if (OB_WRP != 0x00000000UL)
    {
      if ((OB_WRP & WRP_MASK_LOW) != 0x00000000UL)
      {
    ...
    if (OB_WRP != 0x00000000UL)
    {
      if ((OB_WRP & WRP_MASK_HIGH) != 0x00000000UL)
      {
        if (NewState != OB_WRPSTATE_DISABLE)
        {
