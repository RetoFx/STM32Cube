    ...
    if ((hcomp->Instance) == COMP2)
    {
         ...
         (hcomp->Init.InvertingInput == COMP_INPUT_MINUS_3_4VREFINT)   )
      {
        ...
        if (comp_voltage_scaler_not_initialized != 0U)
        {
          ...
          wait_loop_index = (COMP_DELAY_VOLTAGE_SCALER_STAB_US * (SystemCoreClock / (1000000UL * 2U)));
...
HAL_StatusTypeDef HAL_COMP_Start(COMP_HandleTypeDef * const hcomp)
{
  ...
  else
  {
    ...
    if(hcomp->State == HAL_COMP_STATE_READY)
    {
      ...
      /* Note: Variable divided by 2 to compensate partially                  */
      /*       CPU processing cycles.                                         */
      wait_loop_index = (COMP_DELAY_STARTUP_US * (SystemCoreClock / (1000000UL * 2U)));
      while(wait_loop_index != 0U)
      {
        wait_loop_index--;
      }
...
void HAL_COMP_IRQHandler(COMP_HandleTypeDef * const hcomp)
{
  ...
  if(READ_BIT(EXTI->PR, exti_line) != RESET)
  {
    ...
    if(READ_BIT(COMP12_COMMON->CSR, COMP_CSR_WINMODE) != 0U)
    {
