...
static HAL_StatusTypeDef  CRYP_EncryptDecrypt_IT(CRYP_HandleTypeDef * const hcryp);
static void               CRYP_SetInitVector(CRYP_HandleTypeDef * const hcryp, uint8_t const * const InitVector);
static void               CRYP_SetKey(CRYP_HandleTypeDef * const hcryp, uint8_t const * const Key);
...
HAL_StatusTypeDef HAL_CRYP_AESECB_Encrypt(CRYP_HandleTypeDef * const hcryp, uint8_t const * const pPlainData, uint16_t Size, uint8_t * const pCypherData, uint32_t Timeout)
{
  ...
  if((((uint32_t)pPlainData & (uint32_t)0x00000003UL) != 0U) || (((uint32_t)pCypherData & (uint32_t)0x00000003UL) != 0U) || ((Size & (uint16_t)0x000FU) != 0U))
  {
    /* Process Locked */
    __HAL_UNLOCK(hcryp);
...
static HAL_StatusTypeDef CRYP_ProcessData(CRYP_HandleTypeDef * const hcryp, uint8_t const * const Input, uint16_t Ilength, uint8_t * const Output, uint32_t Timeout)
{
  ...
  for(index=0; (index < Ilength); index += 16U)
  {
    /* Write the Input block in the Data Input register */
    hcryp->Instance->DINR = *(uint32_t *)(inputaddr);
    inputaddr += 4U;
