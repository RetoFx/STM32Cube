...
static void IRDA_SetConfig(IRDA_HandleTypeDef * const hirda);
static HAL_StatusTypeDef IRDA_Transmit_IT(IRDA_HandleTypeDef * const hirda);
static HAL_StatusTypeDef IRDA_EndTransmit_IT(IRDA_HandleTypeDef * const hirda);
static HAL_StatusTypeDef IRDA_Receive_IT(IRDA_HandleTypeDef * const hirda);
...
static HAL_StatusTypeDef IRDA_WaitOnFlagUntilTimeout(IRDA_HandleTypeDef * const hirda, uint32_t Flag, FlagStatus Status, uint32_t Tickstart, uint32_t Timeout);
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_IRDA_RegisterCallback(IRDA_HandleTypeDef * const hirda, HAL_IRDA_CallbackIDTypeDef CallbackID, pIRDA_CallbackTypeDef pCallback)
{
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_IRDA_Transmit(IRDA_HandleTypeDef * const hirda, uint8_t const * pData, uint16_t Size, uint32_t Timeout)
{
  uint16_t const * tmp;
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_IRDA_Receive(IRDA_HandleTypeDef * const hirda, uint8_t * pData, uint16_t Size, uint32_t Timeout)
{
  uint16_t * tmp;
  uint32_t tickstart = 0U;
...
HAL_StatusTypeDef HAL_IRDA_Transmit_DMA(IRDA_HandleTypeDef * const hirda, uint8_t const * const pData, uint16_t Size)
{
  ...
  if (hirda->gState == HAL_IRDA_STATE_READY)
  {
    ...
    HAL_DMA_Start_IT(hirda->hdmatx, *(uint32_t const *)tmp, (uint32_t)&hirda->Instance->DR, Size);
...
static HAL_StatusTypeDef IRDA_Transmit_IT(IRDA_HandleTypeDef * const hirda)
{
  ...
  if (hirda->gState == HAL_IRDA_STATE_BUSY_TX)
  {
    if (hirda->Init.WordLength == IRDA_WORDLENGTH_9B)
    {
      tmp = (uint16_t const *) hirda->pTxBuffPtr;
