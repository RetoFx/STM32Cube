...
/**
  ...
  * @retval None
  */
__weak void HAL_OPAMP_MspInit(OPAMP_HandleTypeDef * const hopamp)
{
...
/**
  ...
  * @retval None
  */
__weak void HAL_OPAMP_MspDeInit(OPAMP_HandleTypeDef * const hopamp)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hopamp);
...
HAL_StatusTypeDef HAL_OPAMP_SelfCalibrate(OPAMP_HandleTypeDef * const hopamp)
{
  ...
  else
  {
    ...
    if(hopamp->State == HAL_OPAMP_STATE_READY)
    {
      ...
      {
        tmp_opamp_otr_otuser = 0x00000000UL;
      ...
      if(opamp_trimmingvaluep == 0x1FU)  /* 0x1FU is not functional */
      {
        opamp_trimmingvaluep = 30U;
      }

      if(opamp_trimmingvaluen == 0x1FU)  /* 0x1FU is not functional */
      {
