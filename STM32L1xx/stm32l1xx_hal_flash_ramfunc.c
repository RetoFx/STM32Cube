...
__RAM_FUNC HAL_StatusTypeDef HAL_FLASHEx_EraseParallelPage(uint32_t Page_Address1, uint32_t Page_Address2)
{
  ...
  if(status == HAL_OK)
  {
    ...
    *(__IO uint32_t *)Page_Address1 = 0x00000000UL;
    /* Write 00000000h to the first word of the second program page to erase */
    *(__IO uint32_t *)Page_Address2 = 0x00000000UL;
...
/**
  ...
  * @retval HAL status
  */
__RAM_FUNC HAL_StatusTypeDef HAL_FLASHEx_ProgramParallelHalfPage(uint32_t Address1, uint32_t const * pBuffer1, uint32_t Address2, uint32_t const * pBuffer2)
{
...
/**
  ...
  * @retval HAL status
  */
__RAM_FUNC HAL_StatusTypeDef HAL_FLASHEx_HalfPageProgram(uint32_t Address, uint32_t const * pBuffer)
{
  uint32_t primask_bit;
  uint32_t count = 0U;
  HAL_StatusTypeDef status = HAL_OK;
...
@endif
  ...
  * @retval HAL Status
  */
__RAM_FUNC HAL_StatusTypeDef HAL_FLASHEx_GetError(uint32_t * const Error)
{
...
__RAM_FUNC HAL_StatusTypeDef HAL_FLASHEx_DATAEEPROM_ProgramDoubleWord(uint32_t Address, uint64_t Data)
{
  ...
  if(status == HAL_OK)
  {
     ...
     *(__IO uint32_t *)Address = (uint32_t) Data;
     Address += 4U;
     *(__IO uint32_t *)Address = (uint32_t) (Data >> 32U);
