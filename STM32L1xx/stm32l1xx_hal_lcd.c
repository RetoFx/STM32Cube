...
HAL_StatusTypeDef HAL_LCD_Init(LCD_HandleTypeDef * const hlcd)
{
  ...
  /* Configure the LCD Prescaler, Divider, Blink mode and Blink Frequency:
     ...
     Set HD[0U] bit according to hlcd->Init.HighDrive value */
...
/**
  ...
  * @retval None
  */
 __weak void HAL_LCD_MspDeInit(LCD_HandleTypeDef * const hlcd)
{
...
/**
  ...
  * @retval None
  */
 __weak void HAL_LCD_MspInit(LCD_HandleTypeDef * const hlcd)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hlcd);
