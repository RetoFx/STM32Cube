...
static HAL_StatusTypeDef  FLASH_OB_WRPConfig(FLASH_OBProgramInitTypeDef const * const pOBInit, FunctionalState NewState);
...
static HAL_StatusTypeDef  FLASH_OB_PCROPConfig(FLASH_AdvOBProgramInitTypeDef * const pAdvOBInit, FunctionalState NewState);
...
@verbatim
  ...
  ==============================================================================
    ...
    (+) HAL_FLASHEx_Erase_IT: end of erase is done when HAL_FLASH_EndOfOperationCallback
        is called with parameter 0xFFFFFFFFUL
...
/**
  ...
  *         (0xFFFFFFFFUL means that all the pages have been correctly erased)
  *
...
/**
  ...
  *   This parameter can be one of the following values:
  *     @arg @ref OB_STOP_NO_RST No reset generated when entering in STOP
  *     @arg @ref OB_STOP_RST Reset generated when entering in STOP
  * @param  OB_STDBY Reset event when entering Standby mode.
  *   This parameter can be one of the following values:
  *     @arg @ref OB_STDBY_NO_RST No reset generated when entering in STANDBY
...
static HAL_StatusTypeDef FLASH_DATAEEPROM_FastProgramByte(uint32_t Address, uint8_t Data)
{
  ...
  if(status == HAL_OK)
  {
    ...
    {
      tmpaddr = Address & 0xFFFFFFFCUL;
      tmp = * (__IO uint32_t *) tmpaddr;
      tmpaddr = 0xFFUL << ((uint32_t) (0x8U * (Address & 0x3U)));
      tmp &= ~tmpaddr;
      status = HAL_FLASHEx_DATAEEPROM_Erase(FLASH_TYPEERASEDATA_WORD, Address & 0xFFFFFFFCUL);
      /* Process Unlocked */
      __HAL_UNLOCK(&pFlash);
      status = HAL_FLASHEx_DATAEEPROM_Program(FLASH_TYPEPROGRAMDATA_FASTWORD, (Address & 0xFFFFFFFCUL), tmp);
      /* Process Locked */
      __HAL_LOCK(&pFlash);
    }
#else /*!Cat1*/
  ...
  if(status == HAL_OK)
  {
    ...
    else
    {
      ...
      if((Address & 0x3U) != 0x3U)
      {
        tmpaddr = Address & 0xFFFFFFFCUL;
        tmp = * (__IO uint32_t *) tmpaddr;
        tmpaddr = 0xFFFFUL << ((uint32_t) (0x8U * (Address & 0x3U)));
        tmp &= ~tmpaddr;
        status = HAL_FLASHEx_DATAEEPROM_Erase(FLASH_TYPEERASEDATA_WORD, Address & 0xFFFFFFFCUL);
        status = HAL_FLASHEx_DATAEEPROM_Program(FLASH_TYPEPROGRAMDATA_FASTWORD, (Address & 0xFFFFFFFCUL), tmp);
      }
...
void FLASH_PageErase(uint32_t PageAddress)
{
  ...
  *(__IO uint32_t *)(uint32_t)(PageAddress & ~(FLASH_PAGE_SIZE - 1U)) = 0x00000000UL;
}
