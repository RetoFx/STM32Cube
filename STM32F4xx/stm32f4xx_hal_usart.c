...
void USART_InitCallbacksToDefault(USART_HandleTypeDef * const husart);
#endif /* USE_HAL_USART_REGISTER_CALLBACKS */
static void USART_EndTxTransfer(USART_HandleTypeDef * const husart);
static void USART_EndRxTransfer(USART_HandleTypeDef * const husart);
static HAL_StatusTypeDef USART_Transmit_IT(USART_HandleTypeDef * const husart);
static HAL_StatusTypeDef USART_EndTransmit_IT(USART_HandleTypeDef * const husart);
static HAL_StatusTypeDef USART_Receive_IT(USART_HandleTypeDef * const husart);
static HAL_StatusTypeDef USART_TransmitReceive_IT(USART_HandleTypeDef * const husart);
static void USART_SetConfig(USART_HandleTypeDef * const husart);
static void USART_DMATransmitCplt(DMA_HandleTypeDef * const hdma);
static void USART_DMATxHalfCplt(DMA_HandleTypeDef * const hdma);
static void USART_DMAReceiveCplt(DMA_HandleTypeDef * const hdma);
static void USART_DMARxHalfCplt(DMA_HandleTypeDef * const hdma);
static void USART_DMAError(DMA_HandleTypeDef * const hdma);
static void USART_DMAAbortOnError(DMA_HandleTypeDef * const hdma);
static void USART_DMATxAbortCallback(DMA_HandleTypeDef * const hdma);
static void USART_DMARxAbortCallback(DMA_HandleTypeDef * const hdma);

static HAL_StatusTypeDef USART_WaitOnFlagUntilTimeout(USART_HandleTypeDef * const husart, uint32_t Flag, FlagStatus Status,
...
/**
  ...
  * @retval None
  */
__weak void HAL_USART_MspInit(USART_HandleTypeDef * const husart)
{
...
/**
  ...
  * @retval None
  */
__weak void HAL_USART_MspDeInit(USART_HandleTypeDef * const husart)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(husart);
  /* NOTE: This function should not be modified, when the callback is needed,
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_USART_Transmit(USART_HandleTypeDef * const husart, uint8_t const * const pTxData, uint16_t Size, uint32_t Timeout)
{
  ...
  if (husart->State == HAL_USART_STATE_READY)
  {
    ...
    while (husart->TxXferCount > 0U)
    {
      ...
      {
        husart->Instance->DR = (uint16_t)(*ptxdata16bits & (uint16_t)0x01FFU);
        ptxdata16bits++;
      }
      else
      {
        husart->Instance->DR = (uint8_t)(*ptxdata8bits & (uint8_t)0xFFU);
        ptxdata8bits++;
      }
  ...
  if (husart->State == HAL_USART_STATE_READY)
  {
    ...
    while (husart->RxXferCount > 0U)
    {
      ...
      * can be written for all the cases. */
      ...
      husart->Instance->DR = (DUMMY_DATA & (uint16_t)0x0FFU);
      ...
      {
        *prxdata16bits = (uint16_t)(husart->Instance->DR & (uint16_t)0x01FFU);
        prxdata16bits++;
      }
      else
      {
        ...
        {
          *prxdata8bits = (uint8_t)(husart->Instance->DR & (uint8_t)0x0FFU);
        }
        else
        {
          *prxdata8bits = (uint8_t)(husart->Instance->DR & (uint8_t)0x07FU);
        }
        prxdata8bits++;
      }
      husart->RxXferCount--;
...
HAL_StatusTypeDef HAL_USART_Transmit_DMA(USART_HandleTypeDef * const husart, uint8_t const * const pTxData, uint16_t Size)
{
  ...
  if (husart->State == HAL_USART_STATE_READY)
  {
    ...
    HAL_DMA_Start_IT(husart->hdmatx, *(uint32_t const *)tmp, (uint32_t)&husart->Instance->DR, Size);
                                                ...
                                                uint16_t Size)
{
  ...
  if (husart->State == HAL_USART_STATE_READY)
  {
    ...
    /* Enable the USART receive DMA stream */
    tmp = (uint32_t *)&pRxData;
    HAL_DMA_Start_IT(husart->hdmarx, (uint32_t)&husart->Instance->DR, *(uint32_t const *)tmp, Size);
...
/**
  ...
  * @retval None
  */
HAL_USART_WEAK_CALLBACK(void, HAL_USART_TxCpltCallback)(USART_HandleTypeDef * const husart)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(husart);
  /* NOTE: This function should not be modified, when the callback is needed,
...
/**
  ...
  * @retval None
  */
HAL_USART_WEAK_CALLBACK(void, HAL_USART_TxHalfCpltCallback)(USART_HandleTypeDef * const husart)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(husart);
  /* NOTE: This function should not be modified, when the callback is needed,
...
/**
  ...
  * @retval None
  */
HAL_USART_WEAK_CALLBACK(void, HAL_USART_RxCpltCallback)(USART_HandleTypeDef * const husart)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(husart);
  /* NOTE: This function should not be modified, when the callback is needed,
...
/**
  ...
  * @retval None
  */
HAL_USART_WEAK_CALLBACK(void, HAL_USART_RxHalfCpltCallback)(USART_HandleTypeDef * const husart)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(husart);
  /* NOTE: This function should not be modified, when the callback is needed,
...
/**
  ...
  * @retval None
  */
HAL_USART_WEAK_CALLBACK(void, HAL_USART_TxRxCpltCallback)(USART_HandleTypeDef * const husart)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(husart);
  /* NOTE: This function should not be modified, when the callback is needed,
...
/**
  ...
  * @retval None
  */
HAL_USART_WEAK_CALLBACK(void, HAL_USART_ErrorCallback)(USART_HandleTypeDef * const husart)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(husart);
  /* NOTE: This function should not be modified, when the callback is needed,
...
/**
  ...
  * @retval None
  */
HAL_USART_WEAK_CALLBACK(void, HAL_USART_AbortCpltCallback)(USART_HandleTypeDef * const husart)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(husart);
...
static void USART_DMATransmitCplt(DMA_HandleTypeDef * const hdma)
{
  USART_HandleTypeDef * const husart = (USART_HandleTypeDef *)((DMA_HandleTypeDef *)hdma)->Parent;
  /* DMA Normal mode */
...
static HAL_StatusTypeDef USART_Receive_IT(USART_HandleTypeDef * const husart)
{
  ...
  if (husart->State == HAL_USART_STATE_BUSY_RX)
  {
    if ((husart->Init.WordLength == USART_WORDLENGTH_9B) && (husart->Init.Parity == USART_PARITY_NONE))
    {
      ...
      *pdata16bits = (uint16_t)(husart->Instance->DR & (uint16_t)0x01FFU);
    ...
    else
    {
      ...
      if ((husart->Init.WordLength == USART_WORDLENGTH_9B) || ((husart->Init.WordLength == USART_WORDLENGTH_8B) && (husart->Init.Parity == USART_PARITY_NONE)))
      {
        *pdata8bits = (uint8_t)(husart->Instance->DR & (uint8_t)0x00FFU);
      }
      else
      {
        *pdata8bits = (uint8_t)(husart->Instance->DR & (uint8_t)0x007FU);
      }
  ...
  if (husart->State == HAL_USART_STATE_BUSY_TX_RX)
  {
    if (husart->TxXferCount != 0x00U)
    {
      if (__HAL_USART_GET_FLAG(husart, USART_FLAG_TXE) != RESET)
      {
        if ((husart->Init.WordLength == USART_WORDLENGTH_9B) && (husart->Init.Parity == USART_PARITY_NONE))
        {
          pdatatx16bits = (uint16_t const *) husart->pTxBuffPtr;
          husart->Instance->DR = (uint16_t)(*pdatatx16bits & (uint16_t)0x01FFU);
          husart->pTxBuffPtr += 2U;
        }
        else
        {
    ...
    if (husart->RxXferCount != 0x00U)
    {
      if (__HAL_USART_GET_FLAG(husart, USART_FLAG_RXNE) != RESET)
      {
        if ((husart->Init.WordLength == USART_WORDLENGTH_9B) && (husart->Init.Parity == USART_PARITY_NONE))
        {
          ...
          *pdatarx16bits = (uint16_t)(husart->Instance->DR & (uint16_t)0x01FFU);
          husart->pRxBuffPtr += 2U;
        }
        else
        {
