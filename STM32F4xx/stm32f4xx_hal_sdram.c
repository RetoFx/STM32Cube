...
/** @addtogroup SDRAM_Private_Functions SDRAM Private Functions
  * @{
  */
static void SDRAM_DMACplt(DMA_HandleTypeDef * const hdma);
static void SDRAM_DMACpltProt(DMA_HandleTypeDef * const hdma);
static void SDRAM_DMAError(DMA_HandleTypeDef * const hdma);
/**
  * @}
  */
...
/**
  ...
  * @retval None
  */
__weak void HAL_SDRAM_MspInit(SDRAM_HandleTypeDef * const hsdram)
{
...
/**
  ...
  * @retval None
  */
__weak void HAL_SDRAM_MspDeInit(SDRAM_HandleTypeDef * const hsdram)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hsdram);
...
/**
  ...
  * @retval None
  */
HAL_SDRAM_WEAK_CALLBACK(void, HAL_SDRAM_RefreshErrorCallback)(SDRAM_HandleTypeDef * const hsdram)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hsdram);
...
/**
  ...
  * @retval None
  */
HAL_SDRAM_WEAK_CALLBACK(void, HAL_SDRAM_DMA_XferCpltCallback)(DMA_HandleTypeDef * const hdma)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hdma);
...
/**
  ...
  * @retval None
  */
HAL_SDRAM_WEAK_CALLBACK(void, HAL_SDRAM_DMA_XferErrorCallback)(DMA_HandleTypeDef * const hdma)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hdma);
...
{
  uint32_t size;
  __IO uint8_t const * pSdramAddress = (uint8_t const *)pAddress;
                                     ...
                                     uint32_t BufferSize)
{
  ...
  uint8_t const * psrcbuff = pSrcBuffer;
                                     ...
                                     uint32_t BufferSize)
{
  ...
  else if ((state == HAL_SDRAM_STATE_READY) || (state == HAL_SDRAM_STATE_WRITE_PROTECTED))
  {
    ...
    {
      *pdestbuff = (uint16_t)((*pSdramAddress) & 0x0000FFFFUL);
      pdestbuff++;
      *pdestbuff = (uint16_t)(((*pSdramAddress) & 0xFFFF0000UL) >> 16U);
      pdestbuff++;
      pSdramAddress++;
    }
                                      ...
                                      uint32_t BufferSize)
{
  ...
  else if (hsdram->State == HAL_SDRAM_STATE_READY)
  {
    ...
    if ((BufferSize % 2U) != 0U)
    {
      *psdramaddress = ((uint32_t)(*psrcbuff) & 0x0000FFFFUL) | ((*psdramaddress) & 0xFFFF0000UL);
    }
...
static void SDRAM_DMACplt(DMA_HandleTypeDef * const hdma)
{
  SDRAM_HandleTypeDef * const hsdram = (SDRAM_HandleTypeDef *)(hdma->Parent);
