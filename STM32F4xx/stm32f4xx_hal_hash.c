...
/** @defgroup HASH_Digest_Calculation_Status HASH Digest Calculation Status
  * @{
  */
#define HASH_DIGEST_CALCULATION_NOT_STARTED       ((uint32_t)0x00000000UL) /*!< DCAL not set after input data written in DIN register */
#define HASH_DIGEST_CALCULATION_STARTED           ((uint32_t)0x00000001UL) /*!< DCAL set after input data written in DIN register     */
/**
  * @}
  */
...
static HAL_StatusTypeDef HASH_IT(HASH_HandleTypeDef * const hhash);
static uint32_t HASH_Write_Block_Data(HASH_HandleTypeDef * const hhash);
static HAL_StatusTypeDef HMAC_Processing(HASH_HandleTypeDef * const hhash, uint32_t Timeout);
/**
  * @}
  */
...
/**
  ...
  * @retval None
  */
__weak void HAL_HASH_MspInit(HASH_HandleTypeDef * const hhash)
{
...
/**
  ...
  * @retval None
  */
__weak void HAL_HASH_MspDeInit(HASH_HandleTypeDef * const hhash)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hhash);
...
/**
  ...
  * @retval None
  */
HAL_HASH_WEAK_CALLBACK(void, HAL_HASH_InCpltCallback)(HASH_HandleTypeDef * const hhash)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hhash);
...
/**
  ...
  * @retval None
  */
HAL_HASH_WEAK_CALLBACK(void, HAL_HASH_DgstCpltCallback)(HASH_HandleTypeDef * const hhash)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hhash);
...
/**
  ...
  * @retval None
  */
HAL_HASH_WEAK_CALLBACK(void, HAL_HASH_ErrorCallback)(HASH_HandleTypeDef * const hhash)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hhash);
...
static void HASH_DMAXferCplt(DMA_HandleTypeDef * const hdma)
{
  HASH_HandleTypeDef * const hhash = (HASH_HandleTypeDef *)((DMA_HandleTypeDef *)hdma)->Parent;
  uint32_t inputaddr;
  uint32_t buffersize;
...
static HAL_StatusTypeDef HASH_WriteData(HASH_HandleTypeDef * const hhash, uint8_t const * const pInBuffer, uint32_t Size)
{
  ...
  }   /* for(buffercounter = 0; buffercounter < Size; buffercounter+= 4U)                 */
...
{
  uint8_t const * pInBuffer_tmp;  /* input data address, input parameter of HASH_WriteData()         */
  ...
  if ((State_tmp == HAL_HASH_STATE_READY) || (State_tmp == HAL_HASH_STATE_SUSPENDED))
  {
    ...
    if (hhash->Phase == HAL_HASH_PHASE_READY)
    {
      ...
      pInBuffer_tmp = (uint8_t const *)pInBuffer;   /* pInBuffer_tmp is set to the input data address */
    ...
    else if (hhash->Phase == HAL_HASH_PHASE_PROCESS)
    {
      ...
      if (hhash->State == HAL_HASH_STATE_SUSPENDED)
      {
        ...
        pInBuffer_tmp = (uint8_t const *)hhash->pHashInBuffPtr;
      ...
      else
      {
        /* pInBuffer_tmp and Size_tmp are initialized to be used afterwards as
        input parameters of HASH_WriteData() */
        pInBuffer_tmp = (uint8_t const *)pInBuffer;
        Size_tmp = Size;
        /* Configure the number of valid bits in last word of the message */
        __HAL_HASH_SET_NBVALIDBITS(Size);
      }
  ...
  if ((State_tmp == HAL_HASH_STATE_READY) || (State_tmp == HAL_HASH_STATE_SUSPENDED))
  {
    ...
    if (hhash->State == HAL_HASH_STATE_SUSPENDED)
    {
      ...
      /* Since this is resumption, pInBuffer_tmp and Size_tmp are not set
         to the API input parameters but to those saved beforehand by HASH_WriteData()
         when the processing was suspended */
      pInBuffer_tmp = (uint8_t const *)hhash->pHashInBuffPtr;  /* pInBuffer_tmp is set to the input data address */
