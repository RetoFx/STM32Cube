    ...
    [..]  This section describe The AES Counter with Cipher Block Chaining-Message
      ...
      (#)  Specific parameters for CCM  :
         ...
         (##) B1 block (header) : associated data length(a) concatenated with Associated Data (A)
            ...
            (+++)  If 216-28 < a < 232, then it is encoded as 0xffU || 0xfeU || [a]32, i.e. six octets
            (+++)  If 232 < a < 264, then it is encoded as 0xffU || 0xffU || [a]64, i.e. ten octets
         (##) CTRx block  : control blocks
            ...
            (+++) Generation of CTR0: same as CTR1 with bit[0U] set to zero.
  ...
  *** Callback registration ***
  ...
  Table 1. Initial Counter Block (ICB)
         ...
         127              95                63            31       0
              ...
              31 ... 0     CRYP_IV0L[31:0]     ICB[31:0], where 32-bit counter= 0x2U

  Table 2.  GCM last block definition

          +-------------------------------------------------------------------+
          |  Bit[0U]   |  Bit[32U]           |  Bit[64U]  | Bit[96U]              |
          |-----------|--------------------|-----------|----------------------|
          |   0x0U     | Header length[31:0]|     0x0U   | Payload length[31:0] |
          |-----------|--------------------|-----------|----------------------|
...
#define  CRYP_PHASE_READY                0x00000001UL /*!< CRYP peripheral is ready for initialization. */
#define  CRYP_PHASE_PROCESS              0x00000002UL /*!< CRYP peripheral is in processing phase */

#if defined(AES)
#define CRYP_OPERATINGMODE_ENCRYPT                   0x00000000UL     /*!< Encryption mode(Mode 1)  */
...
#define CRYP_PHASE_INIT                              0x00000000UL        /*!< GCM/GMAC (or CCM) init phase */
...
#define CRYP_OPERATINGMODE_ENCRYPT                   0x00000000UL             /*!< Encryption mode   */
...
#define CRYP_CCM_CTR1_0                  0x07FFFFFFUL
#define CRYP_CCM_CTR1_1                  0xFFFFFF00UL
#define CRYP_CCM_CTR1_2                  0x00000001UL
...
static void CRYP_SetDMAConfig(CRYP_HandleTypeDef * const hcryp, uint32_t inputaddr, uint16_t Size, uint32_t outputaddr);
static void CRYP_DMAInCplt(DMA_HandleTypeDef * const hdma);
static void CRYP_DMAOutCplt(DMA_HandleTypeDef * const hdma);
static void CRYP_DMAError(DMA_HandleTypeDef * const hdma);
static void CRYP_SetKey(CRYP_HandleTypeDef * const hcryp, uint32_t KeySize);
static void CRYP_AES_IT(CRYP_HandleTypeDef * const hcryp);
#if defined (CRYP_CR_ALGOMODE_AES_GCM)|| defined (AES)
static HAL_StatusTypeDef CRYP_GCMCCM_SetHeaderPhase(CRYP_HandleTypeDef * const hcryp, uint32_t Timeout);
static void CRYP_GCMCCM_SetPayloadPhase_IT(CRYP_HandleTypeDef * const hcryp);
static void CRYP_GCMCCM_SetHeaderPhase_IT(CRYP_HandleTypeDef * const hcryp);
static HAL_StatusTypeDef CRYP_GCMCCM_SetHeaderPhase_DMA(CRYP_HandleTypeDef * const hcryp);
static void CRYP_Workaround(CRYP_HandleTypeDef * const hcryp, uint32_t Timeout);
static HAL_StatusTypeDef CRYP_AESGCM_Process_DMA(CRYP_HandleTypeDef * const hcryp);
static HAL_StatusTypeDef CRYP_AESGCM_Process_IT(CRYP_HandleTypeDef * const hcryp);
static HAL_StatusTypeDef CRYP_AESGCM_Process(CRYP_HandleTypeDef * const hcryp, uint32_t Timeout);
static HAL_StatusTypeDef CRYP_AESCCM_Process(CRYP_HandleTypeDef * const hcryp, uint32_t Timeout);
static HAL_StatusTypeDef CRYP_AESCCM_Process_IT(CRYP_HandleTypeDef * const hcryp);
static HAL_StatusTypeDef CRYP_AESCCM_Process_DMA(CRYP_HandleTypeDef * const hcryp);
#endif /* AES or GCM CCM defined*/
static void CRYP_AES_ProcessData(CRYP_HandleTypeDef * const hcrypt, uint32_t Timeout);
static HAL_StatusTypeDef CRYP_AES_Encrypt(CRYP_HandleTypeDef * const hcryp, uint32_t Timeout);
static HAL_StatusTypeDef CRYP_AES_Decrypt(CRYP_HandleTypeDef * const hcryp, uint32_t Timeout);
static HAL_StatusTypeDef CRYP_AES_Decrypt_IT(CRYP_HandleTypeDef * const hcryp);
static HAL_StatusTypeDef CRYP_AES_Encrypt_IT(CRYP_HandleTypeDef * const hcryp);
static HAL_StatusTypeDef CRYP_AES_Decrypt_DMA(CRYP_HandleTypeDef * const hcryp);
#if defined (CRYP)
static void CRYP_TDES_IT(CRYP_HandleTypeDef * const hcryp);
#if defined (CRYP_CR_ALGOMODE_AES_GCM)
static HAL_StatusTypeDef CRYP_WaitOnIFEMFlag(CRYP_HandleTypeDef const * const hcryp, uint32_t Timeout);
#endif /* GCM CCM defined*/
static HAL_StatusTypeDef CRYP_WaitOnBUSYFlag(CRYP_HandleTypeDef const * const hcryp, uint32_t Timeout);
static HAL_StatusTypeDef CRYP_WaitOnOFNEFlag(CRYP_HandleTypeDef const * const hcryp, uint32_t Timeout);
static HAL_StatusTypeDef CRYP_TDES_Process(CRYP_HandleTypeDef * const hcryp, uint32_t Timeout);
#else /*AES*/
...
/**
  ...
  * @retval None
  */
__weak void HAL_CRYP_MspInit(CRYP_HandleTypeDef * const hcryp)
{
...
/**
  ...
  * @retval None
  */
__weak void HAL_CRYP_MspDeInit(CRYP_HandleTypeDef * const hcryp)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hcryp);
...
/**
  ...
  * @retval status
  */
HAL_StatusTypeDef HAL_CRYP_RegisterCallback(CRYP_HandleTypeDef * const hcryp, HAL_CRYP_CallbackIDTypeDef CallbackID,
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_CRYP_Encrypt(CRYP_HandleTypeDef * const hcryp, uint32_t const * const Input, uint16_t Size, uint32_t * const Output,
                                   uint32_t Timeout)
{
  uint32_t algo;
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_CRYP_Decrypt(CRYP_HandleTypeDef * const hcryp, uint32_t const * const Input, uint16_t Size, uint32_t * const Output,
                                   uint32_t Timeout)
{
  HAL_StatusTypeDef status;
  uint32_t algo;
...
/**
  ...
  * @retval None
  */
HAL_CRYPT_WEAK_CALLBACK(void, HAL_CRYP_InCpltCallback)(CRYP_HandleTypeDef * const hcryp)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hcryp);
...
/**
  ...
  * @retval None
  */
HAL_CRYPT_WEAK_CALLBACK(void, HAL_CRYP_OutCpltCallback)(CRYP_HandleTypeDef * const hcryp)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hcryp);
...
/**
  ...
  * @retval None
  */
HAL_CRYPT_WEAK_CALLBACK(void, HAL_CRYP_ErrorCallback)(CRYP_HandleTypeDef * const hcryp)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hcryp);
...
static HAL_StatusTypeDef CRYP_TDES_Process(CRYP_HandleTypeDef * const hcryp, uint32_t Timeout)
{
  uint32_t temp[2U];  /* Temporary CrypOutBuff */
...
static void CRYP_DMAInCplt(DMA_HandleTypeDef * const hdma)
{
  CRYP_HandleTypeDef * const hcryp = (CRYP_HandleTypeDef *)((DMA_HandleTypeDef *)hdma)->Parent;
...
/**
  ...
  * @retval None
  */
static void CRYP_AES_ProcessData(CRYP_HandleTypeDef * const hcryp, uint32_t Timeout)
{

  uint32_t temp[4U];  /* Temporary CrypOutBuff */
  uint32_t i;
#if defined (CRYP)
  uint16_t incount;  /* Temporary CrypInCount Value */
  uint16_t outcount;  /* Temporary CrypOutCount Value */
...
static HAL_StatusTypeDef CRYP_AESCCM_Process(CRYP_HandleTypeDef * const hcryp, uint32_t Timeout)
{
  ...
  if (DoKeyIVConfig == 1U)
  {
    ...
    hcryp->Instance->IV0LR = (hcryp->Init.B0[0U]) & CRYP_CCM_CTR1_0;
    hcryp->Instance->IV0RR = hcryp->Init.B0[1U];
    hcryp->Instance->IV1LR = hcryp->Init.B0[2U];
    hcryp->Instance->IV1RR = (hcryp->Init.B0[3U] & CRYP_CCM_CTR1_1) |  CRYP_CCM_CTR1_2;
...
static void CRYP_Workaround(CRYP_HandleTypeDef * const hcryp, uint32_t Timeout)
{
  ...
  uint32_t  temp2[4U] = {0};
#endif /* CRYP */
  uint32_t intermediate_data[4U] = {0};
  uint32_t index;
  ...
  if (hcryp->Init.Algorithm == CRYP_AES_GCM)
  {
    ...
    if ((hcryp->Instance->CR & CRYP_CR_ALGODIR) == CRYP_OPERATINGMODE_ENCRYPT)
    {
      ...
      if ((hcryp->Instance->CR & CRYP_CR_DATATYPE) == CRYP_DATATYPE_32B)
      {
        if ((npblb % 4U) == 1U)
        {
          intermediate_data[lastwordsize - 1U] &= 0xFFFFFF00UL;
        }
        if ((npblb % 4U) == 2U)
        {
          intermediate_data[lastwordsize - 1U] &= 0xFFFF0000UL;
        }
        if ((npblb % 4U) == 3U)
        {
          intermediate_data[lastwordsize - 1U] &= 0xFF000000UL;
        }
      }
      else if ((hcryp->Instance->CR & CRYP_CR_DATATYPE) == CRYP_DATATYPE_8B)
      {
        if ((npblb % 4U) == 1U)
        {
          intermediate_data[lastwordsize - 1U] &= __REV(0xFFFFFF00UL);
        }
        if ((npblb % 4U) == 2U)
        {
          intermediate_data[lastwordsize - 1U] &= __REV(0xFFFF0000UL);
        }
        if ((npblb % 4U) == 3U)
        {
          intermediate_data[lastwordsize - 1U] &= __REV(0xFF000000UL);
        }
      }
      else if ((hcryp->Instance->CR & CRYP_CR_DATATYPE) == CRYP_DATATYPE_16B)
      {
        if ((npblb % 4U) == 1U)
        {
          intermediate_data[lastwordsize - 1U] &= __ROR((0xFFFFFF00UL), 16);
        }
        if ((npblb % 4U) == 2U)
        {
          intermediate_data[lastwordsize - 1U] &= __ROR((0xFFFF0000UL), 16);
        }
        if ((npblb % 4U) == 3U)
        {
          intermediate_data[lastwordsize - 1U] &= __ROR((0xFF000000UL), 16);
        }
      }
      else /*CRYP_DATATYPE_1B*/
      {
        if ((npblb % 4U) == 1U)
        {
          intermediate_data[lastwordsize - 1U] &= __RBIT(0xFFFFFF00UL);
        }
        if ((npblb % 4U) == 2U)
        {
          intermediate_data[lastwordsize - 1U] &= __RBIT(0xFFFF0000UL);
        }
        if ((npblb % 4U) == 3U)
        {
          intermediate_data[lastwordsize - 1U] &= __RBIT(0xFF000000UL);
        }
      }
  ...
  else
  {
    ...
    if ((hcryp->Instance->CR & CRYP_CR_ALGODIR) == CRYP_OPERATINGMODE_DECRYPT)
    {
      ...
      temp[0U] =  hcryp->Instance->CSGCMCCM0R;
      temp[1U] =  hcryp->Instance->CSGCMCCM1R;
      temp[2U] =  hcryp->Instance->CSGCMCCM2R;
      temp[3U] =  hcryp->Instance->CSGCMCCM3R;
    ...
    if ((hcryp->Instance->CR & CRYP_CR_ALGODIR) == CRYP_OPERATINGMODE_DECRYPT)
    {
      temp2[0U] =  hcryp->Instance->CSGCMCCM0R;
      temp2[1U] =  hcryp->Instance->CSGCMCCM1R;
      temp2[2U] =  hcryp->Instance->CSGCMCCM2R;
      temp2[3U] =  hcryp->Instance->CSGCMCCM3R;
