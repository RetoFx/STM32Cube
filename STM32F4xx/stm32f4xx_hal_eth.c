...
/** @addtogroup ETH_Private_Constants ETH Private Constants
  * @{
  */
#define ETH_MACCR_MASK          0xFFFB7F7CUL
#define ETH_MACECR_MASK         0x3F077FFFUL
#define ETH_MACFFR_MASK         0x800007FFUL
#define ETH_MACWTR_MASK         0x0000010FUL
#define ETH_MACTFCR_MASK        0xFFFF00F2UL
#define ETH_MACRFCR_MASK        0x00000003UL
#define ETH_MTLTQOMR_MASK       0x00000072UL
#define ETH_MTLRQOMR_MASK       0x0000007BUL

#define ETH_DMAMR_MASK          0x00007802UL
#define ETH_DMASBMR_MASK        0x0000D001UL
#define ETH_DMACCR_MASK         0x00013FFFUL
#define ETH_DMACTCR_MASK        0x003F1010UL
#define ETH_DMACRCR_MASK        0x803F0000UL
...
#define ETH_MACTSCR_MASK        0x0087FF2FUL

#define ETH_PTPTSHR_VALUE       0xFFFFFFFFUL
#define ETH_PTPTSLR_VALUE       0xBB9ACA00UL
...
#define ETH_MACCR_CLEAR_MASK    0xFD20810FUL
...
/** @defgroup ETH_Private_Functions   ETH Private Functions
  * @{
  */
static void ETH_SetMACConfig(ETH_HandleTypeDef * const heth, ETH_MACConfigTypeDef const * const macconf);
static void ETH_SetDMAConfig(ETH_HandleTypeDef * const heth, ETH_DMAConfigTypeDef const * const dmaconf);
static void ETH_MACDMAConfig(ETH_HandleTypeDef * const heth);
static void ETH_DMATxDescListInit(ETH_HandleTypeDef * const heth);
static void ETH_DMARxDescListInit(ETH_HandleTypeDef * const heth);
static uint32_t ETH_Prepare_Tx_Descriptors(ETH_HandleTypeDef * const heth, ETH_TxPacketConfigTypeDef const * const pTxConfig,
                                           uint32_t ItMode);
static void ETH_UpdateDescriptor(ETH_HandleTypeDef * const heth);
static void ETH_FlushTransmitFIFO(ETH_HandleTypeDef * const heth);
static void ETH_MACAddressConfig(ETH_HandleTypeDef * const heth, uint32_t MacAddr, uint8_t const * const Addr);

#if (USE_HAL_ETH_REGISTER_CALLBACKS == 1U)
static void ETH_InitCallbacksToDefault(ETH_HandleTypeDef * const heth);
...
/**
  ...
  * @retval None
  */
__weak void HAL_ETH_MspInit(ETH_HandleTypeDef * const heth)
{
...
/**
  ...
  * @retval None
  */
__weak void HAL_ETH_MspDeInit(ETH_HandleTypeDef * const heth)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(heth);
  /* NOTE : This function Should not be modified, when the callback is needed,
...
HAL_StatusTypeDef HAL_ETH_ReleaseTxPacket(ETH_HandleTypeDef * const heth)
{
  ...
  ETH_TimeStampTypeDef * const timestamp = &heth->TxTimestamp;
...
HAL_StatusTypeDef HAL_ETH_PTP_SetConfig(ETH_HandleTypeDef * const heth, ETH_PTP_ConfigTypeDef * const ptpconfig)
{
  ...
  if (ptpconfig->TimestampAddendUpdate == ENABLE)
  {
    ...
    while ((heth->Instance->PTPTSCR & ETH_PTPTSCR_TSARU) != 0U)
    {
...
/**
  ...
  * @retval None
  */
__weak void HAL_ETH_TxPtpCallback(uint32_t *buff, ETH_TimeStampTypeDef * const timestamp)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(buff);
  /* NOTE : This function Should not be modified, when the callback is needed,
...
/**
  ...
  * @retval None
  */
HAL_ETH_WEAK_CALLBACK(void, HAL_ETH_TxCpltCallback)(ETH_HandleTypeDef * const heth)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(heth);
  /* NOTE : This function Should not be modified, when the callback is needed,
...
/**
  ...
  * @retval None
  */
HAL_ETH_WEAK_CALLBACK(void, HAL_ETH_RxCpltCallback)(ETH_HandleTypeDef * const heth)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(heth);
  /* NOTE : This function Should not be modified, when the callback is needed,
...
/**
  ...
  * @retval None
  */
HAL_ETH_WEAK_CALLBACK(void, HAL_ETH_ErrorCallback)(ETH_HandleTypeDef * const heth)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(heth);
  /* NOTE : This function Should not be modified, when the callback is needed,
...
/**
  ...
  * @retval None
  */
__weak void HAL_ETH_PMTCallback(ETH_HandleTypeDef * const heth)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(heth);
  /* NOTE : This function Should not be modified, when the callback is needed,
...
/**
  ...
  * @retval None
  */
__weak void HAL_ETH_WakeUpCallback(ETH_HandleTypeDef * const heth)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(heth);
  /* NOTE : This function Should not be modified, when the callback is needed,
...
HAL_StatusTypeDef HAL_ETH_GetMACConfig(ETH_HandleTypeDef const * const heth, ETH_MACConfigTypeDef * const macconf)
{
  ...
  /* Get MAC parameters */
  macconf->DeferralCheck = ((READ_BIT(heth->Instance->MACCR, ETH_MACCR_DC) >> 4U) > 0U) ? ENABLE : DISABLE;
  macconf->BackOffLimit = READ_BIT(heth->Instance->MACCR, ETH_MACCR_BL);
  macconf->RetryTransmission = ((READ_BIT(heth->Instance->MACCR, ETH_MACCR_RD) >> 9U) == 0U) ? ENABLE : DISABLE;
  macconf->CarrierSenseDuringTransmit = ((READ_BIT(heth->Instance->MACCR, ETH_MACCR_CSD) >> 16U) > 0U)
                                        ? ENABLE : DISABLE;
  macconf->ReceiveOwn = ((READ_BIT(heth->Instance->MACCR, ETH_MACCR_ROD) >> 13U) == 0U) ? ENABLE : DISABLE;
  macconf->LoopbackMode = ((READ_BIT(heth->Instance->MACCR, ETH_MACCR_LM) >> 12U) > 0U) ? ENABLE : DISABLE;
  macconf->DuplexMode = READ_BIT(heth->Instance->MACCR, ETH_MACCR_DM);
  macconf->Speed = READ_BIT(heth->Instance->MACCR, ETH_MACCR_FES);
  macconf->Jabber = ((READ_BIT(heth->Instance->MACCR, ETH_MACCR_JD) >> 22U) == 0U) ? ENABLE : DISABLE;
  macconf->Watchdog = ((READ_BIT(heth->Instance->MACCR, ETH_MACCR_WD) >> 23) == 0U) ? ENABLE : DISABLE;
  macconf->AutomaticPadCRCStrip = ((READ_BIT(heth->Instance->MACCR, ETH_MACCR_APCS) >> 7U) > 0U) ? ENABLE : DISABLE;
  macconf->InterPacketGapVal = READ_BIT(heth->Instance->MACCR, ETH_MACCR_IFG);
  macconf->ChecksumOffload = ((READ_BIT(heth->Instance->MACCR, ETH_MACCR_IPCO) >> 10U) > 0U) ? ENABLE : DISABLE;
  macconf->CRCStripTypePacket = ((READ_BIT(heth->Instance->MACCR, ETH_MACCR_CSTF) >> 25U) > 0U) ? ENABLE : DISABLE;

  macconf->TransmitFlowControl = ((READ_BIT(heth->Instance->MACFCR, ETH_MACFCR_TFCE) >> 1U) > 0U) ? ENABLE : DISABLE;
  macconf->ZeroQuantaPause = ((READ_BIT(heth->Instance->MACFCR, ETH_MACFCR_ZQPD) >> 7U) == 0U) ? ENABLE : DISABLE;
  macconf->PauseLowThreshold = READ_BIT(heth->Instance->MACFCR, ETH_MACFCR_PLT);
  macconf->PauseTime = (READ_BIT(heth->Instance->MACFCR, ETH_MACFCR_PT) >> 16U);
...
HAL_StatusTypeDef HAL_ETH_GetDMAConfig(ETH_HandleTypeDef const * const heth, ETH_DMAConfigTypeDef * const dmaconf)
{
  ...
  dmaconf->EnhancedDescriptorFormat = ((READ_BIT(heth->Instance->DMABMR, ETH_DMABMR_EDE) >> 7U) > 0U) ? ENABLE : DISABLE;
  dmaconf->DescriptorSkipLength = READ_BIT(heth->Instance->DMABMR, ETH_DMABMR_DSL) >> 2U;

  dmaconf->DropTCPIPChecksumErrorFrame = ((READ_BIT(heth->Instance->DMAOMR,
                                                    ETH_DMAOMR_DTCEFD) >> 26) > 0U) ? DISABLE : ENABLE;
  dmaconf->ReceiveStoreForward = ((READ_BIT(heth->Instance->DMAOMR, ETH_DMAOMR_RSF) >> 25U) > 0U) ? ENABLE : DISABLE;
  dmaconf->FlushRxPacket = ((READ_BIT(heth->Instance->DMAOMR, ETH_DMAOMR_FTF) >> 20U) > 0U) ? DISABLE : ENABLE;
  dmaconf->TransmitStoreForward = ((READ_BIT(heth->Instance->DMAOMR, ETH_DMAOMR_TSF) >> 21U) > 0U) ? ENABLE : DISABLE;
  dmaconf->TransmitThresholdControl = READ_BIT(heth->Instance->DMAOMR, ETH_DMAOMR_TTC);
  dmaconf->ForwardErrorFrames = ((READ_BIT(heth->Instance->DMAOMR, ETH_DMAOMR_FEF) >> 7U) > 0U) ? ENABLE : DISABLE;
  dmaconf->ForwardUndersizedGoodFrames = ((READ_BIT(heth->Instance->DMAOMR,
                                                    ETH_DMAOMR_FUGF) >> 6U) > 0U) ? ENABLE : DISABLE;
  dmaconf->ReceiveThresholdControl = READ_BIT(heth->Instance->DMAOMR, ETH_DMAOMR_RTC);
  dmaconf->SecondFrameOperate = ((READ_BIT(heth->Instance->DMAOMR, ETH_DMAOMR_OSF) >> 2U) > 0U) ? ENABLE : DISABLE;
...
void HAL_ETH_SetMDIOClockRange(ETH_HandleTypeDef * const heth)
{
  ...
  else if (hclk < 60000000UL)
  {
    /* CSR Clock Range between 35-60 MHz */
    tmpreg |= (uint32_t)ETH_MACMIIAR_CR_Div26;
  }
  else if (hclk < 100000000UL)
  {
    /* CSR Clock Range between 60-100 MHz */
    tmpreg |= (uint32_t)ETH_MACMIIAR_CR_Div42;
  }
  else if (hclk < 150000000UL)
  {
    /* CSR Clock Range between 100-150 MHz */
    tmpreg |= (uint32_t)ETH_MACMIIAR_CR_Div62;
  }
  else /* (hclk >= 150000000UL)  */
  {
    /* CSR Clock >= 150 MHz */
...
HAL_StatusTypeDef HAL_ETH_SetMACFilterConfig(ETH_HandleTypeDef * const heth, ETH_MACFilterConfigTypeDef const * const pFilterConfig)
{
  ...
  filterconfig = ((uint32_t)pFilterConfig->PromiscuousMode |
                  ((uint32_t)pFilterConfig->HashUnicast << 1U) |
                  ((uint32_t)pFilterConfig->HashMulticast << 2U)  |
                  ((uint32_t)pFilterConfig->DestAddrInverseFiltering << 3U) |
                  ((uint32_t)pFilterConfig->PassAllMulticast << 4U) |
                  ((uint32_t)((pFilterConfig->BroadcastFilter == DISABLE) ? 1U : 0U) << 5U) |
                  ((uint32_t)pFilterConfig->SrcAddrInverseFiltering << 8U) |
                  ((uint32_t)pFilterConfig->SrcAddrFiltering << 9U) |
                  ((uint32_t)pFilterConfig->HachOrPerfectFilter << 10U) |
...
HAL_StatusTypeDef HAL_ETH_GetMACFilterConfig(ETH_HandleTypeDef const * const heth, ETH_MACFilterConfigTypeDef * const pFilterConfig)
{
  ...
  pFilterConfig->HashUnicast = ((READ_BIT(heth->Instance->MACFFR, ETH_MACFFR_HU) >> 1U) > 0U) ? ENABLE : DISABLE;
  pFilterConfig->HashMulticast = ((READ_BIT(heth->Instance->MACFFR, ETH_MACFFR_HM) >> 2U) > 0U) ? ENABLE : DISABLE;
  pFilterConfig->DestAddrInverseFiltering = ((READ_BIT(heth->Instance->MACFFR,
                                                       ETH_MACFFR_DAIF) >> 3U) > 0U) ? ENABLE : DISABLE;
  pFilterConfig->PassAllMulticast = ((READ_BIT(heth->Instance->MACFFR, ETH_MACFFR_PAM) >> 4U) > 0U) ? ENABLE : DISABLE;
  pFilterConfig->BroadcastFilter = ((READ_BIT(heth->Instance->MACFFR, ETH_MACFFR_BFD) >> 5U) == 0U) ? ENABLE : DISABLE;
  ...
  pFilterConfig->SrcAddrInverseFiltering = ((READ_BIT(heth->Instance->MACFFR,
                                                      ETH_MACFFR_SAIF) >> 8U) > 0U) ? ENABLE : DISABLE;
  pFilterConfig->SrcAddrFiltering = ((READ_BIT(heth->Instance->MACFFR, ETH_MACFFR_SAF) >> 9U) > 0U) ? ENABLE : DISABLE;
  pFilterConfig->HachOrPerfectFilter = ((READ_BIT(heth->Instance->MACFFR, ETH_MACFFR_HPF) >> 10U) > 0U)
                                       ? ENABLE : DISABLE;
  pFilterConfig->ReceiveAllMode = ((READ_BIT(heth->Instance->MACFFR, ETH_MACFFR_RA) >> 31U) > 0U) ? ENABLE : DISABLE;
                                                ...
                                                uint8_t const * pMACAddr)
{
  ...
  (*(__IO uint32_t *)macaddrhr) = (((uint32_t)(pMACAddr[5U]) << 8U) | (uint32_t)pMACAddr[4U]);
  /* Set MAC addr bits 0 to 31 */
  (*(__IO uint32_t *)macaddrlr) = (((uint32_t)(pMACAddr[3U]) << 24U) | ((uint32_t)(pMACAddr[2U]) << 16U) |
                                   ((uint32_t)(pMACAddr[1U]) << 8U) | (uint32_t)pMACAddr[0U]);
...
HAL_StatusTypeDef HAL_ETH_SetHashTable(ETH_HandleTypeDef * const heth, uint32_t * pHashTable)
{
  ...
  heth->Instance->MACHTHR = pHashTable[0U];
  ...
  heth->Instance->MACHTLR = pHashTable[1U];
...
/**
  ...
  * @retval None
  */
void HAL_ETH_SetRxVLANIdentifier(ETH_HandleTypeDef * const heth, uint32_t ComparisonBits, uint32_t VLANIdentifier)
{
  uint32_t tmpreg1;
...
/**
  ...
  * @retval None.
  */
void HAL_ETH_EnterPowerDownMode(ETH_HandleTypeDef * const heth, ETH_PowerDownConfigTypeDef const * const pPowerDownConfig)
{
                                           ...
                                           uint32_t ItMode)
{
  ...
  ETH_BufferTypeDef const * txbuffer = pTxConfig->TxBuffer;
  uint32_t           bd_count = 0;
