...
/** @addtogroup DMA_Private_Functions
  * @{
  */
static void DMA_SetConfig(DMA_HandleTypeDef * const hdma, uint32_t SrcAddress, uint32_t DstAddress, uint32_t DataLength);
static uint32_t DMA_CalcBaseAndBitshift(DMA_HandleTypeDef * const hdma);
static HAL_StatusTypeDef DMA_CheckFifoParam(DMA_HandleTypeDef * const hdma);
...
HAL_StatusTypeDef HAL_DMA_DeInit(DMA_HandleTypeDef * const hdma)
{
  ...
  hdma->Instance->FCR  = 0x00000021UL;
...
static uint32_t DMA_CalcBaseAndBitshift(DMA_HandleTypeDef * const hdma)
{
  ...
  static uint8_t const flagBitshiftOffset[8U] = {0U, 6U, 16U, 22U, 0U, 6U, 16U, 22U};
