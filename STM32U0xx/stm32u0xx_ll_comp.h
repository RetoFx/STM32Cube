...
__STATIC_INLINE uint32_t LL_COMP_GetCommonWindowMode(COMP_Common_TypeDef const * const COMPxy_COMMON)
{
  /* Note: On this STM32 series, window mode can be set from any instance     */
  /*       of the pair of comparator instances.                               */
  register uint32_t const window_mode_comp_odd = (uint32_t)READ_BIT(COMPxy_COMMON->CSR_ODD, COMP_CSR_WINMODE);
  register uint32_t const window_mode_comp_even = (uint32_t)READ_BIT(COMPxy_COMMON->CSR_EVEN, COMP_CSR_WINMODE);
...
/**
  ...
  * @retval None
  */
__STATIC_INLINE void LL_COMP_SetCommonWindowOutput(COMP_Common_TypeDef * const COMPxy_COMMON, uint32_t WindowOutput)
{
  __IO uint32_t * preg = __COMP_PTR_REG_OFFSET(COMPxy_COMMON->CSR_ODD,
...
/**
  ...
  *         @arg @ref LL_COMP_WINDOWOUTPUT_COMP2
  */
__STATIC_INLINE uint32_t LL_COMP_GetCommonWindowOutput(COMP_Common_TypeDef const * const COMPxy_COMMON)
{
  register uint32_t const window_output_comp_odd = (uint32_t)READ_BIT(COMPxy_COMMON->CSR_ODD, COMP_CSR_WINOUT);
  register uint32_t const window_output_comp_even = (uint32_t)READ_BIT(COMPxy_COMMON->CSR_EVEN, COMP_CSR_WINOUT);
