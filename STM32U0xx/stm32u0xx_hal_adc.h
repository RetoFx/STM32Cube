...
#endif /* USE_HAL_ADC_REGISTER_CALLBACKS */
{
  ...
  void (* CalibrationCpltCallback)(struct  __ADC_HandleTypeDef * const hadc);      /*!< ADC end of calibration callback */
  void (* ADCReadyCallback)(struct   __ADC_HandleTypeDef * const hadc);            /*!< ADC Ready callback */
  void (* ChannelConfigReadyCallback)(struct __ADC_HandleTypeDef * const hadc);    /*!< ADC Channel Configuration Ready callback */
...
/** @defgroup ADC_HAL_EC_REG_SEQ_RANKS  ADC group regular - Sequencer ranks
  * @{
  */
#define ADC_RANK_CHANNEL_NUMBER            (0x00000001UL)        /*!< Enable the rank of the selected channels. Number of
                                           ...
                                           configurable. */
#define ADC_RANK_NONE                      (0x00000002UL)        /*!< Disable the selected rank (selected channel) from
                                           sequencer.
                                           Setting relevant if parameter "ScanConvMode" is set to sequencer not fully
                                           configurable. */
...
#define ADC_SCAN_SEQ_FIXED_INT  0x80000000UL
...
void                    HAL_ADC_CalibrationCpltCallback(ADC_HandleTypeDef * const hadc);
void                    HAL_ADC_ADCReadyCallback(ADC_HandleTypeDef * const hadc);
/**
  * @}
  */
...
/* Private functions ---------------------------------------------------------*/
HAL_StatusTypeDef ADC_ConversionStop(ADC_HandleTypeDef * const hadc);
