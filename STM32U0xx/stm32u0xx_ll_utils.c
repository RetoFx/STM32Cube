...
/** @addtogroup UTILS_LL_Private_Constants
  * @{
  */
#define UTILS_MAX_FREQUENCY_SCALE1  56000000UL        /*!< Maximum frequency for system clock at power scale1, in Hz */
#define UTILS_MAX_FREQUENCY_SCALE2  18000000UL        /*!< Maximum frequency for system clock at power scale2, in Hz */
...
#define UTILS_PLLVCO_OUTPUT_MIN     96000000UL        /*!< Frequency min for PLLVCO output, in Hz  */
#define UTILS_PLLVCO_OUTPUT_MAX     344000000UL       /*!< Frequency max for PLLVCO output, in Hz  */

/* Defines used for FLASH latency according to HCLK Frequency */
#define UTILS_SCALE1_LATENCY1_FREQ  24000000UL        /*!< HCLK frequency to set FLASH latency 1 in power scale 1 */
#define UTILS_SCALE1_LATENCY2_FREQ  48000000UL        /*!< HCLK frequency to set FLASH latency 2 in power scale 1 */
#define UTILS_SCALE1_LATENCY3_FREQ  56000000UL        /*!< HCLK frequency to set FLASH latency 3 in power scale 1 */
...
#define UTILS_HCLK_MAX              56000000UL        /*!< HCLK frequency maximum at 64MHz */
...
/**
  ...
  *         - PLLR:ensure that max frequency at 122000000UL Hz is reached (PLLVCO_output / PLLR)
...
/**
  ...
  *         - PLLN:ensure that the VCO output frequency is between 96 and 344 MHz (PLLVCO_output = PLLVCO_input * PLLN)
  *         - PLLR:ensure that max frequency at 122000000UL Hz is reach (PLLVCO_output / PLLR)
  * @param  UTILS_PLLInitStruct pointer to a @ref LL_UTILS_PLLInitTypeDef structure that contains
  *                             the configuration information for the PLL.
  * @param  UTILS_ClkInitStruct pointer to a @ref LL_UTILS_ClkInitTypeDef structure that contains
  *                             the configuration information for the BUS prescalers.
...
/**
  ...
  *          - ERROR: Max frequency configuration not done
  */
ErrorStatus LL_PLL_ConfigSystemClock_HSE(uint32_t HSEBypass, LL_UTILS_PLLInitTypeDef const * const UTILS_PLLInitStruct,
                                         LL_UTILS_ClkInitTypeDef const * const UTILS_ClkInitStruct)
{
  ErrorStatus status = SUCCESS;
  uint32_t pllrfreq = 0U;
...
static uint32_t UTILS_GetPLLOutputFrequency(uint32_t PLL_InputFrequency, LL_UTILS_PLLInitTypeDef const * const UTILS_PLLInitStruct)
{
  ...
  /*  - PLLR: ensure that max frequency at 64000000UL Hz is reached                   */
