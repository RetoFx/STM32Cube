...
#define RTC_ALARMSUBSECONDBIN_AUTOCLR_YES   RTC_ALRMASSR_SSCLR   /*!<  The synchronous Binary counter
                                                                       (SS[31:0] in RTC_SSR) is running from 0xFFFFFFFFUL
                                                                       to RTC_ALRMABINR -> SS[31:0] value and is
                                                                       automatically reloaded with 0xFFFFFFFFUL
...
#define __HAL_RTC_GET_FLAG(__HANDLE__, __FLAG__) \
  (((((__FLAG__)) >> 8U) == 1U) ? (RTC->ICSR & (1UL << (((uint16_t)(__FLAG__)) & RTC_IT_MASK))) : \
   (RTC->SR & (1UL << (((uint16_t)(__FLAG__)) & RTC_IT_MASK))))
...
HAL_StatusTypeDef HAL_RTCEx_GetTimeStamp(RTC_HandleTypeDef const * const hrtc, RTC_TimeTypeDef * const sTimeStamp,
                                         RTC_DateTypeDef * const sTimeStampDate, uint32_t Format);
void              HAL_RTCEx_TimeStampIRQHandler(RTC_HandleTypeDef * const hrtc);
...
HAL_StatusTypeDef HAL_RTCEx_SetSmoothCalib(RTC_HandleTypeDef * const hrtc, uint32_t SmoothCalibPeriod,
...
/** @defgroup RTCEx_Exported_Functions_Group5 Extended RTC Tamper functions
  * @{
  */
HAL_StatusTypeDef HAL_RTCEx_SetTamper(RTC_HandleTypeDef const * const hrtc, RTC_TamperTypeDef const * const sTamper);
HAL_StatusTypeDef HAL_RTCEx_SetTamper_IT(RTC_HandleTypeDef const * const hrtc, RTC_TamperTypeDef const * const sTamper);
HAL_StatusTypeDef HAL_RTCEx_DeactivateTamper(RTC_HandleTypeDef const * const hrtc, uint32_t Tamper);
HAL_StatusTypeDef HAL_RTCEx_PollForTamperEvent(RTC_HandleTypeDef * const hrtc, uint32_t Tamper, uint32_t Timeout);
HAL_StatusTypeDef HAL_RTCEx_SetInternalTamper(RTC_HandleTypeDef const * const hrtc,
                                              RTC_InternalTamperTypeDef const * const sIntTamper);
HAL_StatusTypeDef HAL_RTCEx_SetInternalTamper_IT(RTC_HandleTypeDef const * const hrtc,
                                                 RTC_InternalTamperTypeDef const * const sIntTamper);
HAL_StatusTypeDef HAL_RTCEx_DeactivateInternalTamper(RTC_HandleTypeDef const * const hrtc, uint32_t IntTamper);
HAL_StatusTypeDef HAL_RTCEx_PollForInternalTamperEvent(RTC_HandleTypeDef * const hrtc, uint32_t IntTamper, uint32_t Timeout);
void              HAL_RTCEx_TamperIRQHandler(RTC_HandleTypeDef * const hrtc);
...
void              HAL_RTCEx_Tamper4EventCallback(RTC_HandleTypeDef * const hrtc);
void              HAL_RTCEx_Tamper5EventCallback(RTC_HandleTypeDef * const hrtc);
void              HAL_RTCEx_InternalTamper3EventCallback(RTC_HandleTypeDef * const hrtc);
void              HAL_RTCEx_InternalTamper4EventCallback(RTC_HandleTypeDef * const hrtc);
void              HAL_RTCEx_InternalTamper5EventCallback(RTC_HandleTypeDef * const hrtc);
void              HAL_RTCEx_InternalTamper6EventCallback(RTC_HandleTypeDef * const hrtc);
/**
  * @}
  */
...
void              HAL_RTCEx_BKUPErase(RTC_HandleTypeDef const * const hrtc);
/**
  * @}
  */
