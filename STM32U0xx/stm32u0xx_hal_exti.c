  ...
  if ((pExtiConfig->Line & EXTI_CONFIG) != 0x00U)
  {
    ...
    *regaddr = regval;
    ...
    if ((pExtiConfig->Line & EXTI_GPIO) == EXTI_GPIO)
    {
      ...
      regval &= ~(0xFFUL << (8U * (linepos & 0x03U)));
...
HAL_StatusTypeDef HAL_EXTI_GetConfigLine(EXTI_HandleTypeDef * const hexti, EXTI_ConfigTypeDef * const pExtiConfig)
{
  ...
  if ((pExtiConfig->Line & EXTI_CONFIG) != 0x00U)
  {
    ...
    if ((pExtiConfig->Line & EXTI_GPIO) == EXTI_GPIO)
    {
      assert_param(IS_EXTI_GPIO_PIN(linepos));
      regval = EXTI->EXTICR[linepos >> 2U];
      pExtiConfig->GPIOSel = ((regval << (8U * (3 - (linepos & 0x03U)))) >> 24U);
    }
  }
  else
  {
...
/**
  ...
  * @retval HAL Status.
  */
HAL_StatusTypeDef HAL_EXTI_RegisterCallback(EXTI_HandleTypeDef * const hexti, EXTI_CallbackIDTypeDef CallbackID,
