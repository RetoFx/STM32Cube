...
uint32_t LL_EXTI_DeInit(void)
{
  ...
  LL_EXTI_WriteReg(IMR,   0x1F800000UL);
  /* Event mask register set to default reset values */
  LL_EXTI_WriteReg(EMR,   0x00000000UL);
  /* Rising Trigger selection register set to default reset values */
  LL_EXTI_WriteReg(RTSR,  0x00000000UL);
  /* Falling Trigger selection register set to default reset values */
  LL_EXTI_WriteReg(FTSR,  0x00000000UL);
  /* Software interrupt event register set to default reset values */
  LL_EXTI_WriteReg(SWIER, 0x00000000UL);
  /* Pending register clear */
  LL_EXTI_WriteReg(PR,    0x007FFFFFUL);

#if defined(EXTI_32_63_SUPPORT)
  /* Interrupt mask register 2 set to default reset values */
#if defined(STM32F334x8)
  LL_EXTI_WriteReg(IMR2,        0xFFFFFFFEUL);
#else
  LL_EXTI_WriteReg(IMR2,        0xFFFFFFFCUL);
#endif
  ...
  LL_EXTI_WriteReg(PR2,         0x00000003UL);
