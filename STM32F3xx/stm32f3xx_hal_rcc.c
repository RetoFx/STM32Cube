...
#define RCC_CFGR_HPRE_BIT_NUMBER           POSITION_VAL(RCC_CFGR_HPRE)
#define RCC_CFGR_PPRE1_BIT_NUMBER          POSITION_VAL(RCC_CFGR_PPRE1)
#define RCC_CFGR_PPRE2_BIT_NUMBER          POSITION_VAL(RCC_CFGR_PPRE2)
/**
  * @}
  */
/* Private macro -------------------------------------------------------------*/
...
/** @defgroup RCC_Private_Variables RCC Private Variables
  * @{
  */
static uint8_t const aPLLMULFactorTable[16U] = { 2U,  3U,  4U,  5U,  6U,  7U,  8U,  9U,
                                                 10U, 11U, 12U, 13U, 14U, 15U, 16U, 16U};
static uint8_t const aPredivFactorTable[16U] = { 1U, 2U,  3U,  4U,  5U,  6U,  7U,  8U,
...
HAL_StatusTypeDef HAL_RCC_ClockConfig(RCC_ClkInitTypeDef const * const RCC_ClkInitStruct, uint32_t FLatency)
{
  ...
  SystemCoreClock = HAL_RCC_GetSysClockFreq() >> AHBPrescTable[(RCC->CFGR & RCC_CFGR_HPRE)>> RCC_CFGR_HPRE_BIT_NUMBER];
...
{
  HAL_PERIPH_BIT_BAND_WRITE(RCC->CR, RCC_CSSON_BIT_NUMBER, ENABLE);
}
...
{
  HAL_PERIPH_BIT_BAND_WRITE(RCC->CR, RCC_CSSON_BIT_NUMBER, DISABLE);
}
...
uint32_t HAL_RCC_GetPCLK1Freq(void)
{
  ...
  return (HAL_RCC_GetHCLKFreq() >> APBPrescTable[(RCC->CFGR & RCC_CFGR_PPRE1) >> RCC_CFGR_PPRE1_BIT_NUMBER]);
}
...
uint32_t HAL_RCC_GetPCLK2Freq(void)
{
  ...
  return (HAL_RCC_GetHCLKFreq()>> APBPrescTable[(RCC->CFGR & RCC_CFGR_PPRE2) >> RCC_CFGR_PPRE2_BIT_NUMBER]);
}
...
/**
  ...
  * @retval none
  */
HAL_RCC_WEAK_CALLBACK(void, HAL_RCC_CSSCallback)(void)
{
