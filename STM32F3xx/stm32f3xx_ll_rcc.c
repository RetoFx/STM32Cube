...
static uint16_t const aADCPrescTable[16U]       = {1U, 2U, 4U, 6U, 8U, 10U, 12U, 16U, 32U, 64U, 128U, 256U, 256U, 256U, 256U, 256U};
#endif /* RCC_CFGR2_ADC1PRES || RCC_CFGR2_ADCPRE12 || RCC_CFGR2_ADCPRE34 */
#if defined(RCC_CFGR_SDPRE)
static uint8_t const aSDADCPrescTable[16U]       = {2U, 4U, 6U, 8U, 10U, 12U, 14U, 16U, 20U, 24U, 28U, 32U, 36U, 40U, 44U, 48U};
...
ErrorStatus LL_RCC_DeInit(void)
{
  ...
  /* Reset CFGR register */
  LL_RCC_WriteReg(CFGR, 0x00000000UL);

  /* Reset CFGR2 register */
  LL_RCC_WriteReg(CFGR2, 0x00000000UL);

  /* Reset CFGR3 register */
  LL_RCC_WriteReg(CFGR3, 0x00000000UL);
  ...
  LL_RCC_WriteReg(CIR, 0x00000000UL);
...
uint32_t LL_RCC_GetADCClockFreq(uint32_t ADCxSource)
{
...
#else
  if ((adc_prescaler & 0x0000FFFFUL) == ((uint32_t)0x00000000UL))
  {
  ...
  else
  {
    ...
    adc_frequency = RCC_GetPCLK2ClockFreq(RCC_GetHCLKClockFreq(RCC_GetSystemClockFreq()))
                    / (aADCPrescTable[((adc_prescaler & 0x0000FFFFUL) >> POSITION_VAL(ADCxSource)) & 0xFU]);
  }
#endif /* RCC_CFGR_ADCPRE */
